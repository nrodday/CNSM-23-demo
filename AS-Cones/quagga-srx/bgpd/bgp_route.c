...
bool srx_use_evaluation(struct bgp* bgp)
{
  bool use_evaluation =    srx_config_check(bgp, SRX_CONFIG_EVAL_ORIGIN)
                        ...
                        || srx_config_check(bgp, SRX_CONFIG_EVAL_ASCONE);

  printf("Debug: use_evaluation -  use_evaluation:%u, SRX_CONFIG_EVAL_ASCONE:%u, bgp->srx_config:%u\n", use_evaluation, SRX_CONFIG_EVAL_ASCONE, bgp->srx_config);
...
int bgp_info_set_ignore_flag (struct bgp_info* info)
{
  ...
  // 4. Check the path validation ignore policy, only if not ignored yet.
  if (!ignore && (bgp->srx_config & SRX_CONFIG_EVAL_ASCONE))
  {
    switch (info->val_res_ASCONE)
    {
      case SRx_RESULT_INVALID:
        if (bgp->srx_val_policy & SRX_VAL_POLICY_ASCONE_IGNORE_INVALID)
        {
          ignore = true;
        }
        break;
      case SRx_RESULT_UNKNOWN:
        if (bgp->srx_val_policy & SRX_VAL_POLICY_ASCONE_IGNORE_UNKNOWN)
        {
          ignore = true;
        }
        break;
      case SRx_RESULT_UNVERIFIABLE:
        if (bgp->srx_val_policy & SRX_VAL_POLICY_ASCONE_IGNORE_UNVERIFIABLE)
        {
          ignore = true;
        }
      case SRx_RESULT_UNDEFINED:
        if (bgp->srx_val_policy & SRX_VAL_POLICY_ASCONE_IGNORE_UNDEFINED)
        {
          ignore = true;
        }
        break;
...
SRxResult getInfoToSrxVal (struct bgp_info *info)
{
  ...
  if (info)
  {
    ...
    resVals.asconeResult = info->val_res_ASCONE;
  }
  else
  {
    ...
    resVals.asconeResult = SRx_RESULT_UNDEFINED;
  }

  printf("Debug: getInfoToSrxVal - asconeResult: %u\n", resVals.asconeResult);
...
bool isSrxValChanged (SRxResult old, SRxResult new)
{
  ...
  if (old.asconeResult != new.asconeResult)
    return true;
...
void bgp_info_set_validation_result (struct bgp_info *info,
                                     ...
                                     uint8_t aspaResult, uint8_t asconeResult)

{
  /* An internal error occurred */
  if ((resType & SRX_FLAG_ROA_BGPSEC_ASPA_ASCONE) == 0)
  {
    zlog_debug ("No SRx result provided! - type [0x%X], Update [0x%08X]", 
        resType, info->updateID);
    printf("\nDebug: bgp_info_set_validation_result - NO UPDATE MADE TYPE IS ZERO\n");
  ...
  {
    printf("Debug: ASCONE - bgp_info_set_validation_result is called!\n");
    printf("resType(8 ascone, 4 aspa): %u\n", resType);
    ...
    printf("Debug: bgp_info_set_validation_result - updateID: %08X, oldValResult: %u, asconeResult: %u\n", info->updateID, oldValResult, asconeResult);
    ...
    if ((resType & SRX_FLAG_ASCONE) == SRX_FLAG_ASCONE)
    {
      info->val_res_ASCONE = asconeResult;  // it came from resCallback()
    }
    ...
    printf("Debug: bgp_route.c - ignore flag is set to: %u\n", ignoreChanged);
                                          ...
                                          uint32_t* delta)
{
  ...
  {    
    ...
    // ASCONE result - Only add if ASCONE validation is enabled.
  if ((bgp->srx_config & SRX_CONFIG_EVAL_ASCONE) != 0)
  {    
    switch (valResult.asconeResult)
    {
      case SRx_RESULT_VALID:
        _srx_policy_getLocPrevValue(&bgp->srx_loc_pref_ascone[VAL_LOCPRF_VALID],
                                    &incVal, &decVal);
        break;
      case SRx_RESULT_UNKNOWN:
        _srx_policy_getLocPrevValue(&bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNKNOWN],
                                    &incVal, &decVal);
        break;
      case SRx_RESULT_INVALID:
        _srx_policy_getLocPrevValue(&bgp->srx_loc_pref_ascone[VAL_LOCPRF_INVALID],
                                    &incVal, &decVal);
        break;
      case SRx_RESULT_UNVERIFIABLE:
        _srx_policy_getLocPrevValue(&bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNVERIFIABLE],
                                    &incVal, &decVal);
        break;
      case SRx_RESULT_UNDEFINED:
        _srx_policy_getLocPrevValue(&bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNDEFINED],
                                    &incVal, &decVal);
        break;
      default:
        zlog_err("Invalid validation result type %02X provided for " \
                 "asconevalidation!\n", valResult.asconeResult);
        break;
    }
  }
...
static bool _srx_is_localPrefEnabled (struct bgp* bgp, int validationType)
{
  ...
  switch (validationType)
  {
    ...
    case LOCPRF_TYPE_ASPA:
      ...
      break;
    case LOCPRF_TYPE_ASCONE:
      maxIdx = VAL_LOCPRF_CT_ASCONE;
      locPref      = bgp->srx_loc_pref_ascone;
      break;
    default:
      maxIdx = 0;
  }
...
/**
 * Check if evaluation is enabled for any of origin, bgpsec, or aspa or ascone validation 
...
bool isSetPrefPolicy(struct bgp* bgp)
{
  ...
  {
    doValidation = _srx_is_localPrefEnabled(bgp, LOCPRF_TYPE_BGPSEC); // Can this be correct?
  }
  printf("Debug: Check if doValidation is true: %u,bgp->srx_config: %u, SRX_CONFIG_EVAL_ASCONE: %u\n", doValidation, bgp->srx_config, SRX_CONFIG_EVAL_ASCONE);
  if (!doValidation && ((bgp->srx_config & SRX_CONFIG_EVAL_ASCONE) != 0))
  {
    printf("Debug: ASCONE isSetPrefPolicy - Check if this result is as expected!\n");
    doValidation = _srx_is_localPrefEnabled(bgp, LOCPRF_TYPE_BGPSEC);
  }
	      ...
	      int *paths_eq)
{
  ...
  int ret;
#ifdef USE_SRX
  printf("Debug: bgp_info_cmp - called!\n");
  ...
  *paths_eq = 0;
  ...
  if (use_evaluation)
  {
    ...
    // Retrieve Result
    printf("Debug:bgp_info_cmp - use_evaluation");
    exist_SrxVal = getInfoToSrxVal(exist);
    new_SrxVal   = getInfoToSrxVal(new);

    printf("Debug: EXIST ASCONE-INFO: %u\n",exist_SrxVal.asconeResult);
    printf("Debug: NEW ASCONE-INFO: %u\n",new_SrxVal.asconeResult);
  }
#endif /* USE_SRX */
...
printf("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
  ...
  if (use_evaluation && isSetPrefPolicy(bgp))
  {
    ...
    printf("Debug: %u new_pref doAdd: %u", __FUNCTION__, doAdd);
    
    delta = 0;
    doAdd = srx_calc_loc_prev_delta_value(bgp, exist_SrxVal, &delta);
    printf("Debug: %u exists_pref doAdd: %u", __FUNCTION__, doAdd);
                    ...
                    SRxDefaultResult* defResult, bool doRegisterLocalID)
{
  ...
  if (defResult->result.asconeResult != SRx_RESULT_DONOTUSE)
  {
    info->val_res_ASCONE = defResult->result.asconeResult;
  }
  ...
  if (isConnected (bgp->srxProxy))
  {
    ...
    if (info->localID != 0 || info->updateID != 0)
    {
      ...
      if( CHECK_FLAG (peer->flags, PEER_FLAG_ASCONE_RELATIONSHIP_PROV))
      {
        asPathList.asRelationship = AS_REL_PROVIDER;        
      }
      else if ( CHECK_FLAG (peer->flags, PEER_FLAG_ASCONE_RELATIONSHIP_CUST))
      {
        asPathList.asRelationship = AS_REL_CUSTOMER;        
      }
      else if ( CHECK_FLAG (peer->flags, PEER_FLAG_ASCONE_RELATIONSHIP_SIBL))
      {
        asPathList.asRelationship = AS_REL_SIBLING;        
      }
      else if ( CHECK_FLAG (peer->flags, PEER_FLAG_ASCONE_RELATIONSHIP_LATL))
      {
        asPathList.asRelationship = AS_REL_LATERAL;        
      }
      else
      ...
      if(info->attr->aspath && info->attr->aspath->segments)
      {
        ...
        zlog_debug ("[ ASPA/ASCONE ] AS PathList Info - AS Length: %d  Type: %s  AS relationship: %s ", 
        ...
        printf ("[ ASCONE ] AS PathList Info - AS Length: %d  Type: %s  AS relationship: %s ", 
                asPathList.length, asPathList.asType==2 ? "AS_SEQUENCE": (asPathList.asType==1 ? "AS_SET": "ETC"), 
                asPathList.asRelationship == 2 ? "provider" : (asPathList.asRelationship == 1 ? "customer": \
                (asPathList.asRelationship == 3 ? "sibling": (asPathList.asRelationship == 4 ? "lateral" : "unknown"))));
        ...
        for (csedIdx=0; cseg && csedIdx < cseg->length; csedIdx++)
        {
          ...
          printf ("[ ASCONE ] asPathList.segment[%d].asn: %6d type:%d AS relationship: %d", 
                csedIdx, cseg->as[csedIdx], asPathList.asType, asPathList.asRelationship);
        }
      }
      ...
      bool useAsconeVal   = false;

      printf("Debug: verify_update\n");
      if (CHECK_FLAG(bgp->srx_config, SRX_CONFIG_EVAL_ASCONE))
      {
        printf("Debug: Set local AS!!!!\n");
        printf("Debug: local_as: %u\n", info->peer->local_as);
      }
      ...
      printf("Debug: bgp_route.c - bgp->srx_config: %u\n", bgp->srx_config);
      printf("Debug: bgp_route.c - SRX_CONFIG_EVAL_ASPA: %u\n", SRX_CONFIG_EVAL_ASPA);
      printf("Debug: bgp_route.c - SRX_CONFIG_EVAL_ASCONE: %u\n", SRX_CONFIG_EVAL_ASCONE);
      ...
      if ( CHECK_FLAG(bgp->srx_config, SRX_CONFIG_EVAL_ASCONE))
      {
        ...
        printf("Debug: Set local AS to AS-PATH!!!!\n");
        printf("Debug: local_as: %u\n", info->peer->local_as);
        printf("Debug: asPathList.segments[0]: %u\n", asPathList.segments[0].asn);
        printf("Debug: asPathList.segments[1]: %u\n", asPathList.segments[1].asn);

        // For ASCONE validation we need also the local AS for validating against the POLICY-Object
        // One extra element is added to the AS-PATH at the beginning which contains the local ASN.
        
        /*ASSEGMENT* tmpASSEGMENT = (ASSEGMENT*)calloc(asPathList.length + 1, sizeof(ASSEGMENT));

        int idx;
        for (idx = 0; idx < asPathList.length; idx++)
        {
          tmpASSEGMENT[idx+1] = asPathList.segments[idx];
        }      */
        
        printf("Debug: BGPSEC_local_as: %u\n", bgpsec->local_as);

        bgpsec->local_as = info->peer->local_as;

        /*
        free(asPathList.segments);
        asPathList.segments = tmpASSEGMENT;
        asPathList.length = asPathList.length + 1;
        asPathList.segments[0].asn = info->peer->local_as;
        ...
        int idx;
         for (idx = 0; idx < asPathList.length; idx++)
        {
          printf("Debug: asPathList.segments[%u]: %u\n", idx, asPathList.segments[idx].asn);
        } 

      }
      printf("Debug: bgp_route.c - useAspaVal: %u\n", useAspaVal);
      printf("Debug: bgp_route.c - useAsconeVal: %u\n", useAsconeVal);

      verifyUpdate(bgp->srxProxy, info->localID, useOriginVal, usePathVal, 
                   useAspaVal, useAsconeVal, defResult, prefix, oas, bgpsec, asPathList);
...
// data to the default data.
	      ...
	      || CHECK_FLAG (peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)))
	{
      ...
      defRes->result.asconeResult = bgp->srx_default_asconeVal;
  ...
  new->val_res_ASCONE   = bgp->srx_default_asconeVal;
  printf("Debug: bgp_update_main - new->val_res_ASCONE: %u\n", bgp->srx_default_asconeVal);
                                         ...
                                         struct bgp_info *binfo)
{
  ...
  if ((bgp->srx_config & SRX_CONFIG_DISPLAY_INFO) != 0)
  {
    ...
    if ((bgp->srx_config & SRX_CONFIG_EVAL_ASCONE) != 0)
    {
      // print ASCONE validation.
      srx_validation_vty_short_out(vty, binfo->val_res_ASCONE);
    }
    else
    {
      vty_out (vty, "-");
    ...
    int srxValidation = SRX_CONFIG_EVAL_ORIGIN | SRX_CONFIG_EVAL_PATH 
                        | SRX_CONFIG_EVAL_ASPA | SRX_CONFIG_EVAL_ASCONE;
                                     ...
                                     struct bgp_info *binfo)
{
  ...
  bool asconeVal   = (bgp->srx_config & SRX_CONFIG_EVAL_ASCONE) != 0;
  ...
  if (originVal | bgpsecVal | aspaVal | asconeVal)
  {
    if (CHECK_FLAG (binfo->flags, BGP_INFO_IGNORE))
    {
    ...
    else
    {
      ...
      if (asconeVal)
      {      
        vty_out (vty, "        ascone:   %s%s", INDEX_STR[binfo->val_res_ASCONE], 
                      VTY_NEWLINE);
      }
      else
      {
        vty_out (vty, "        ascone:   processing disabled!%s",
                      VTY_NEWLINE);
      }
    }
		      ...
		      struct bgp_info *binfo, afi_t afi, safi_t safi)
{
  ...
  if (CHECK_FLAG(bgp->srx_config, SRX_CONFIG_DISPLAY_INFO))
  {
	...
	vty_out (vty, ", metric %u", attr->med);
      ...
      printf("Debug: route_vty_out - called!\n");
...
#define BGP_SRX_SHOW_SCODE_HEADER "Validation:    v - valid, i - invalid, n - notfound, u - unknown, f - unverifiable, ? - undefined%s" \
                                  ...
                                  "SRxVal Format: origin validation, path validation, aspa validation, ascone validation%s"  \
