...
bool handleSRxValidationResult (SRxUpdateID updateID, uint32_t localID,
                                ...
                                uint8_t asconeResult, uint8_t aspaResult,
                                void* bgpRouter);
...
{
  if((flags & SRX_FLAG_ASPA)>0){
    printf("WARNING ASPA FLAG IS TOGGLED!!!\n");
    printf("Debug: bgpd.c - srx_config_set: %u\n", flags);
  }
...
int bgp_srx_set_evaluation (struct bgp *bgp, int mode)
{
  ...
  if (mode != 0)
  {
    printf("Debug: bgp_srx_set_evaluation - mode: %u, SRX_CONFIG_EVAL_ASCONE: %u\n", mode, SRX_CONFIG_EVAL_ASCONE);
    ...
    retVal = (mode & SRX_CONFIG_EVAL_ASCONE)     != 0 ? CMD_SUCCESS : retVal;
...
int bgp_srx_conf_default_result(struct bgp *bgp, int type, int def_value)
{
  ...
  switch (type)
  {
    ...
    case SRX_VTY_PARAM_ASPA_VALUE:
      ...
      break;
    case SRX_VTY_PARAM_ASCONE_VALUE:
      printf("Debug: ASCONE SRX_VTY_PARAM_ASCONE_VALUE: %d\n", SRX_VTY_PARAM_ASCONE_VALUE);
      bgp->srx_default_asconeVal = def_value;
      break;
    default:
...
//        bgp->srx_loc_pref_aspa[resultType].value = value;
      ...
      break;
    case VAL_LOCPRF_CT_ASCONE:
      if (valType < VAL_LOCPRF_CT_ASCONE)
      {
        prefPolicy = &(bgp->srx_loc_pref_ascone[resultType]);
//        bgp->srx_loc_pref_aspa[resultType].is_set = (value != 0) ? 1 : 0;
//        bgp->srx_loc_pref_aspa[resultType].relative = relative;
//        bgp->srx_loc_pref_aspa[resultType].value = value;
      }
                                ...
                                uint8_t aspaResult, void* bgpRouter)
{
  ...
  */
  printf("\nDebug: handleSRxValidationResult - updateID: %08X, valType: %u\n", updateID, valType);
                                                ...
                                                struct bgp_table* table)
{
  ...
  for (bnode = bgp_table_top(table); bnode; bnode = bgp_route_next(bnode))
  {
    if (bnode->info != NULL)
    {
      ...
      defResult.resSourceASCONE     = SRxRS_ROUTER;
      ...
      defResult.result.asconeResult = binfo->val_res_ASCONE;
      printf("Debug: bgpd.c - defResult.result.asconeResult: %u\n", binfo->val_res_ASCONE);
...
void srx_set_default(struct bgp *bgp)
{
  ...
  memset (bgp->srx_loc_pref_ascone,   0, structSize * VAL_LOCPRF_CT_ASCONE);
  ...
  bgp->srx_default_asconeVal   = SRx_RESULT_UNDEFINED;

  printf("Debug: bgpd.c - srx_set_default - bgp->srx_config: %u\n", bgp->srx_config);
 ...
 * @param type The type string (roa|bgpsec|aspa|ascone)
...
static int srx_config_write_configuration (struct vty *vty, struct bgp *bgp)
{
                            ...
                            | SRX_CONFIG_EVAL_ASCONE))
  {
    ...
    // Now check for ASCONE validation
    if (srx_config_check(bgp, SRX_CONFIG_EVAL_ASCONE))
    {
      vty_out (vty, " srx evaluation %s%s", SRX_VTY_EVAL_ASCONE, VTY_NEWLINE);
    }
    else
    {
      vty_out (vty, "! Disable default configuration%s", VTY_NEWLINE);
      vty_out (vty, " no srx evaluation %s%s", SRX_VTY_EVAL_ASCONE, VTY_NEWLINE);   
    }
  }
  else
  {
  ...
  switch (bgp->srx_default_bgpsecVal)
  {
    ...
    // srx set-ascone-value
  vty_out (vty, " srx set-ascone-value ");
  switch (bgp->srx_default_bgpsecVal)
  {
    case SRx_RESULT_VALID:
      vty_out (vty, "valid%s", VTY_NEWLINE );
  ...
  // ASCONE
  if (CHECK_FLAG (bgp->srx_val_policy, SRX_VAL_POLICY_ASCONE_IGNORE_INVALID))
  {
    vty_out (vty, " srx policy ascone ignore invalid%s", VTY_NEWLINE);
  }
  if (CHECK_FLAG (bgp->srx_val_policy, SRX_VAL_POLICY_ASCONE_IGNORE_UNKNOWN))
  {
    vty_out (vty, " srx policy ascone ignore unknown%s", VTY_NEWLINE);
  }  
  if (CHECK_FLAG (bgp->srx_val_policy, SRX_VAL_POLICY_ASCONE_IGNORE_UNVERIFIABLE))
  {
    vty_out (vty, " srx policy ascone ignore unverifiable%s", VTY_NEWLINE);
  }
  if (CHECK_FLAG (bgp->srx_val_policy, SRX_VAL_POLICY_ASCONE_IGNORE_UNDEFINED))
  {
    vty_out (vty, " srx policy ascone ignore undefined%s", VTY_NEWLINE);
  }
  else
  {
    vty_out (vty, "! Disable default configuration%s", VTY_NEWLINE);
    vty_out (vty, " no srx policy ascone ignore undefined%s", VTY_NEWLINE);
  }
  ...
  if (hasPolicy) vty_out(vty, "%s", VTY_NEWLINE);
    ...
    // VALIDATION POLICY LOCAL PREV ASCONE ...
  hasPolicy =
    _srx_write_config_loc_pref(vty, &bgp->srx_loc_pref_ascone[VAL_LOCPRF_VALID], 
                             SRX_VTY_EVAL_ASCONE, "valid");
  hasPolicy |= 
    _srx_write_config_loc_pref(vty, &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNKNOWN], 
                             SRX_VTY_EVAL_ASCONE, "unknown");
  hasPolicy |= 
    _srx_write_config_loc_pref(vty, &bgp->srx_loc_pref_ascone[VAL_LOCPRF_INVALID], 
                             SRX_VTY_EVAL_ASCONE, "invalid");
  hasPolicy |= 
    _srx_write_config_loc_pref(vty, &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNVERIFIABLE], 
                             SRX_VTY_EVAL_ASCONE, "unverifiable");
  hasPolicy |= 
    _srx_write_config_loc_pref(vty, &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNDEFINED], 
                             SRX_VTY_EVAL_ASCONE, "undefined");
  if (hasPolicy) vty_out(vty, "%s", VTY_NEWLINE);
