 ...
 * @param typeStr The name of the type e.g. origin, bgpsec, aspa, ascone
       ...
       SRX_VTY_HLP_SHOW_CONFIG)
{
  ...
  bool evalASCONE   = (bgp->srx_config & SRX_CONFIG_EVAL_ASCONE) != 0;
  printf("Debug: bgp_vty.c - evalASCONE: %u, bgp->srx_config: %u, SRX_CONFIG_EVAL_ASCONE: %u\n", evalASCONE, bgp->srx_config, SRX_CONFIG_EVAL_ASCONE);
  ...
  switch (bgp->srx_default_aspaVal)
  {
   ...
   vty_out (vty, "%sascone   validation = ", _SRX_BLANKS);
  switch (bgp->srx_default_asconeVal)
  {
    case SRx_RESULT_VALID:
      vty_out (vty, "valid (v)%s", VTY_NEWLINE); break;
    case SRx_RESULT_INVALID:
  ...
  int ignoreASCONE =  SRX_VAL_POLICY_ASCONE_IGNORE_INVALID
                  | SRX_VAL_POLICY_ASCONE_IGNORE_UNKNOWN
                  | SRX_VAL_POLICY_ASCONE_IGNORE_UNVERIFIABLE
                  | SRX_VAL_POLICY_ASCONE_IGNORE_UNDEFINED;
  // reset for ignore listing
  
  vty_out (vty, "  SRx validation result policies%s", VTY_NEWLINE);
  vty_out (vty, "  - ignore.......: ");
  if (bgp->srx_val_policy & (ignoreROA | ignoreBGPsec | ignoreASPA | ignoreASCONE ))
  {
    // ROA
    ...
    // ASCONE
    if (bgp->srx_val_policy & ignoreASCONE)
    {
      more = false;
      vty_out (vty, "%sasCONE - if validation is ", doPolicy ? _SRX_BLANKS 
                                                                : "");
      ...
      vty_out (vty, "%s%s", evalASCONE ? "" : " (disabled)", VTY_NEWLINE);            
    }
  }
  else
  {
  ...
  for (idx = 0; !doLocPrefASCONE && (idx < VAL_LOCPRF_CT_ASCONE); idx++)
  {
    doLocPrefASCONE= bgp->srx_loc_pref_ascone[idx].enable;
  }
   
  vty_out (vty, "  - local pref...: ");  
  doPolicy = false;
  if (doLocPrefROA || doLocPrefBGPSEC || doLocPrefASPA || doLocPrefASCONE)
  {
    // Check ROA policy
    if (doLocPrefROA)
    {
    ...
    // ASCONE Local Pref Policy    
    if (doLocPrefASCONE)
    {
      doPolicy |= _srx_show_config_local_pref(vty, 
                    &bgp->srx_loc_pref_ascone[VAL_LOCPRF_VALID], doPolicy,
                    SRX_VTY_EVAL_ASCONE, 'v', "valid", evalASCONE);
      doPolicy |= _srx_show_config_local_pref(vty, 
                    &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNKNOWN], doPolicy,
                    SRX_VTY_EVAL_ASCONE, 'u', "unknown", evalASCONE);
      doPolicy |= _srx_show_config_local_pref(vty, 
                    &bgp->srx_loc_pref_ascone[VAL_LOCPRF_INVALID], doPolicy,
                    SRX_VTY_EVAL_ASCONE, 'i', "invalid", evalASCONE);
      doPolicy |= _srx_show_config_local_pref(vty, 
                    &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNVERIFIABLE], doPolicy,
                    SRX_VTY_EVAL_ASCONE, 'f', "unverifiable", evalASCONE);
      doPolicy |= _srx_show_config_local_pref(vty, 
                    &bgp->srx_loc_pref_ascone[VAL_LOCPRF_UNDEFINED], doPolicy,
                    SRX_VTY_EVAL_ASCONE, '?', "undefined", evalASCONE);            
    }
  }
  if (!doPolicy)
  {
       ...
       SRX_VTY_HLP_EVALUATE)
{
  ...
  printf("Debug: ASCOSRX_CONFIG_EVAL_ASCONENE : %s\n", argv[0]);
  // BUG? This was the old behaviour. This set the mode always to 1 and nothing else.
  /*
  ...
  else if (strncmp(argv[0], SRX_VTY_EVAL_ASCONE, 1) == 0)
  {
    mode = SRX_CONFIG_EVAL_ASCONE;
    printf("Debug: ASCONE SRX_CONFIG_EVAL_ASCONE\n");
  }*/

  if (strncmp(argv[0], SRX_VTY_EVAL_ORIGIN, 3) == 0)
  {
    mode = (mode | SRX_CONFIG_EVAL_ORIGIN);
  }
  if (strncmp(argv[0], SRX_VTY_EVAL_BGPSEC, 3) == 0)
  {
    mode = (mode | SRX_CONFIG_EVAL_PATH);
  }
  if (strncmp(argv[0], SRX_VTY_EVAL_ASPA, 3) == 0)
  {
    mode = (mode | SRX_CONFIG_EVAL_ASPA);
  }
  if (strncmp(argv[0], SRX_VTY_EVAL_ASCONE, 3) == 0)
  {
    printf("Debug: srx_evaluation - mode before: %u\n", mode);
    mode = (mode | SRX_CONFIG_EVAL_ASCONE);
    printf("Debug: ASCONE SRX_CONFIG_EVAL_ASCONE mode\n");
  }

  printf("Debug: srx_evaluation - mode: %u\n", mode);
       ...
       SRX_VTY_HLP_NO_EVALUATE)
{
  ...
  printf("Debug: bgp_vty.c - check if this function is correct?\n");
  ...
  if (mode != 0)
  {
    printf("Debug: srx_evaluation - NO mode before: %u\n", mode);
    bgp_srx_unset_evaluation(bgp, mode);
    printf("Debug: srx_evaluation - NO mode after: %u\n", mode);
    retVal = CMD_SUCCESS;
  }
...
//@CHECK default for ASCONE missing here.
...
DEFUN (srx_conf_default_ascone_result,
       srx_conf_default_ascone_result_cmd,
       SRX_VTY_CMD_CONF_DEF_ASCONE_RES,
       SRX_VTY_HLP_CONF_DEF_ASCONE_RES)
{
  struct bgp* bgp;
  int value = -1;
  int retVal = CMD_SUCCESS;
  ...
  if (retVal == CMD_SUCCESS)
  {
    retVal = bgp_srx_conf_default_result (bgp, SRX_VTY_PARAM_ASCONE_VALUE, value);
  }
 ...
 * 
 * @return  LOCPRF_TYPE_ROA, LOCPRF_TYPE_BGPSEC, or LOCPRF_TYPE_ASPA, or LOCPREF_TYPE_ASCONE
 * 
 * @since 0.6.0.0
 */
static int parse_local_pref_type (const char *str)
{
  ...
  printf("Debug: ASCONE THIS function needs to be adapted for ASCONE!!!\n");
    ...
    switch (valType)
    {
      ...
      case LOCPRF_TYPE_ASPA:
        ...
        switch (resultType)
        {
          ...
          case VAL_LOCPRF_UNVERIFIABLE:
          case VAL_LOCPRF_UNDEFINED:
            locPref = &bgp->srx_loc_pref_ascone[resultType];
            break;
          default:
            retVal = CMD_ERR_NO_MATCH;        
        }
...
// LOC_PREF POLICY FOR ASCONE
DEFUN (srx_policy_ascone_local_preference,
       srx_policy_ascone_local_preference_cmd,
       SRX_VTY_CMD_POL_ASCONE_LOCP,
       SRX_VTY_HLP_POL_ASCONE_LOCP
       )
{
  return validation_local_preference(vty->index, 
                                     LOCPRF_TYPE_ASCONE, argc, argv, true);
}

DEFUN (no_srx_policy_ascone_local_preference,
       no_srx_policy_ascone_local_preference_cmd,
       SRX_VTY_NO_CMD_POL_ASCONE_LOCP,
       SRX_VTY_NO_HLP_POL_ASCONE_LOCP
       )
{
  return validation_local_preference(vty->index, 
                                     LOCPRF_TYPE_ASCONE, argc, argv, false);
}
...
// IGNORE POLICY FOR ASCONE
DEFUN (srx_policy_ascone_ignore,
       srx_policy_ascone_ignore_cmd,
       SRX_VTY_CMD_POL_ASCONE_IGNORE,
       SRX_VTY_HLP_POL_ASCONE_IGNORE)
{
  struct bgp* bgp;
  int policy = 0;
  int retVal = CMD_SUCCESS;
  ...
  if (strncmp(argv[0], "unknown", 3) == 0)
  {
    policy = SRX_VAL_POLICY_ASCONE_IGNORE_UNKNOWN;
  }
  else if (strncmp(argv[0], "invalid", 1) == 0)
  {
    policy = SRX_VAL_POLICY_ASCONE_IGNORE_INVALID;
  }
  else if (strncmp(argv[0], "unverifiable", 3) == 0)
  {
    policy = SRX_VAL_POLICY_ASCONE_IGNORE_UNVERIFIABLE;
  }
  else if (strncmp(argv[0], "undefined", 3) == 0)
  {
    policy = SRX_VAL_POLICY_ASCONE_IGNORE_UNDEFINED;
  }
  else
  {
    retVal = CMD_ERR_NO_MATCH;
...
DEFUN (no_srx_policy_ascone_ignore,
       no_srx_policy_ascone_ignore_cmd,
       SRX_VTY_NO_CMD_POL_ASCONE_IGNORE,
       SRX_VTY_NO_HLP_POL_ASCONE_IGNORE)
{
  struct bgp* bgp;
  int policy = 0;
  int retVal = CMD_SUCCESS;
...
// peering relationship for ASCONE
DEFUN (neighbor_ascone_peering_relationship,
       neighbor_ascone_peering_relationship_cmd,
       SRX_VTY_CMD_NEIGHBOR_AS_RELATIONSHIP,
       SRX_VTY_HLP_NEIGHBOR_AS_RELATIONSHIP)
{
  ...
  if (strncmp (argv[1], "p", 1) == 0)
  {
    flag = PEER_FLAG_ASCONE_RELATIONSHIP_PROV;
  }
  else if (strncmp (argv[1], "c", 1) == 0)
  {
    flag = PEER_FLAG_ASCONE_RELATIONSHIP_CUST;
  }
  else if (strncmp (argv[1], "s", 1) == 0)
  {
    flag = PEER_FLAG_ASCONE_RELATIONSHIP_SIBL;
  }
  else if (strncmp (argv[1], "l", 1) == 0)
  {
    flag = PEER_FLAG_ASCONE_RELATIONSHIP_LATL;
  }
  else
  {
    return CMD_ERR_INCOMPLETE;
...
DEFUN (no_neighbor_ascone_peering_relationship,
       no_neighbor_ascone_peering_relationship_cmd,
       SRX_VTY_CMD_NO_NEIGHBOR_AS_RELATIONSHIP,
       SRX_VTY_HLP_NO_NEIGHBOR_AS_RELATIONSHIP)
{
...
bgp_vty_init (void)
{
  ...
  install_element (BGP_NODE, &srx_conf_default_ascone_result_cmd);
...
//  install_element (BGP_NODE, &srx_apply_policy_cmd);
  ...
  // ASCONE LOCAL-PREF POICIES
  install_element (BGP_NODE, &srx_policy_ascone_local_preference_cmd);
  install_element (BGP_NODE, &no_srx_policy_ascone_local_preference_cmd);
  ...
  // ASCONE IGNORE POLICIES  
  install_element (BGP_NODE, &srx_policy_ascone_ignore_cmd);
  install_element (BGP_NODE, &no_srx_policy_ascone_ignore_cmd);
  ...
  /* ascone peering relationship */
  install_element (BGP_NODE, &neighbor_ascone_peering_relationship_cmd);
  install_element (BGP_NODE, &no_neighbor_ascone_peering_relationship_cmd);
