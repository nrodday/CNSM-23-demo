...
typedef struct {
    uint32_t     ASN;
    uint16_t      OtherASCone[255];
    uint16_t      verified;
  } __attribute__((packed)) asConePduEntry;

typedef struct {
 uint8_t     version;                 // Version
 uint8_t     type;                    // TYPE_ASCONE
 uint16_t    zero_1;                  // zero
 uint32_t    length;                  // 160+ Bytes
 uint16_t    customerCount;           // at least one customer must be present
 uint16_t     objectType;              // 0 = ASCONE-Object 1 = ASPOLICY-Object
 //uint16_t    zero_2;                  // zero
 uint32_t    asnCone;                 // ASN of the provider
 uint8_t     coneUniqueName[255];     //Unique Name of the
 uint8_t     coneVersion;             // Version of the AS-Cone
 time_t      coneLastModified;        // Last modification of the AS-Cone
 time_t      coneCreated;             // Creation Date of the AS-Cone
 uint8_t     coneContactEmail[75];    // E-Mail Contact of AS-Cone
 uint16_t    zero_3;                  // zero
 //uint32_t    customerASN;             // ASN of the first customer
 //uint8_t     otherASConeString[255];  // ASCONE needs an identifier which is not clearly specified in ASN.1
 //uint8_t     verified;                // Verified flag of the customer
  // followed by list of customer_asn ((customer_asn + char_array[255] + verified) * customer_as_count))
  // or by a list of neighbours
} __attribute__((packed)) RPKIASCONEHeader;
