...
bool createAspathCache(AspathCache* self, ASPA_DBManager* aspaDBManager, ASCONE_DBManager* asconeDBManager, ASCONE_DBManager* aspolicyDBManager)
{
  ...
  self->asconeDBManager = asconeDBManager;
  self->aspolicyDBManager = aspolicyDBManager;
...
AS_PATH_LIST* newAspathListEntry (uint32_t length, uint32_t* pathData, uint32_t pathId, AS_TYPE asType, 
                                  AS_REL_DIR asRelDir, uint16_t afi, uint32_t localAS, bool bBigEndian)
{
  if (!length)
  {
  ...
  printf("Debug: FIX THIS UGLY HACK!!!");
  //@TODO: FIX THIS
  //pAspathList->localAS      = bBigEndian ? ntohs(localAS): localAS;
  pAspathList->localAS      = ntohl(localAS);
  printf("Debug: newAspathListEntry - localAS: %u\n", localAS);
  printf("Debug: newAspathListEntry - localAS: %u\n", pAspathList->localAS);
                      ...
                      uint8_t modAspaResult, AS_PATH_LIST* pathlistEntry)
{
  ...
  else
  {
    if(modAspaResult != SRx_RESULT_DONOTUSE)
    {
      ...
      LOG(LEVEL_INFO, "ASPA - AspathCache entry for path ID: 0x%08X - last modfied time update: %u", pathId, plCacheTable->lastModified);

      if(modAspaResult != plCacheTable->aspaResult)
      {
        plCacheTable->aspaResult   = modAspaResult;
        LOG(LEVEL_INFO, FILE_LINE_INFO "ASPA AS path cache data modified [pathID]:0x%08X [Value]: %d [Time]: %u", 
...
bool modifyAsconeValidationResultToAspathCache(AspathCache *self, uint32_t pathId,
                      uint8_t modAsconeResult, AS_PATH_LIST* pathlistEntry)
{
  bool retVal = true;
  PathListCacheTable *plCacheTable;
  ...
  {
    if(modAsconeResult != SRx_RESULT_DONOTUSE)
    {
      // access time updated
      plCacheTable->lastModified = pathlistEntry->lastModified;
      LOG(LEVEL_INFO, "ASCONE AspathCache entry for path ID: 0x%08X - last modfied time update: %u", pathId, plCacheTable->lastModified);
      printf("Debug: ASCONE AspathCache entry for path ID: 0x%08X - last modfied time update: %u\n", pathId, plCacheTable->lastModified);

      if(modAsconeResult != plCacheTable->asconeResult)
      {
        plCacheTable->asconeResult   = modAsconeResult;
        printf("Debug: ASCONE AS path cache data modified [pathID]:0x%08X [Value]: %d [Time]: %u\n", 
            pathId, modAsconeResult, plCacheTable->lastModified);
        printf("Debug: modAsconeResult: %u, plCacheTable->asconeResult: %u\n", modAsconeResult, plCacheTable->asconeResult);
                      ...
                      uint32_t pathId, AS_TYPE asType, AS_PATH_LIST* pathlistEntry)
{
  ...
  else
  {
    ...
    plCacheTable->lastModified = pathlistEntry->lastModified;
    printf("Debug: storeAspathList - localAS: %u - pathId: %08X\n", pathlistEntry->localAS, pathId);
    plCacheTable->localAS      = pathlistEntry->localAS;
    ...
    if (srxRes != NULL)
    {
      ...
      plCacheTable->asconeResult = srxRes->result.asconeResult;
    }
...
AS_PATH_LIST* getAspathListFromAspathCache (AspathCache* self, uint32_t pathId, SRxResult* srxRes)
{
  ...
  if (find_AspathList (self, pathId, &plCacheTable))
  {
    ...
    aspl->asconeValResult = plCacheTable->asconeResult;
    ...
    aspl->localAS       = plCacheTable->localAS;

    printf("Debug: getAspathListFromAspathCache - aspl->localAS: %u - pathId: %08X\n", aspl->localAS, aspl->pathID);
    ...
    printf("Debug: aspath_cache.c - getAspathListFromAspathCache\n");
    printf("Debug: aspath_cache.c - getAspathListFromAspathCache - srxRes->aspaResult: %u, srxRes->asconeResult: %u\n", srxRes->aspaResult, srxRes->asconeResult);

    if (srxRes->aspaResult != aspl->aspaValResult)
      srxRes->aspaResult  = aspl->aspaValResult;

    if (srxRes->asconeResult != aspl->asconeValResult)
  ...
  else
  {
    ...
    srxRes->asconeResult = SRx_RESULT_UNDEFINED;
  }
...
uint32_t makePathId (uint8_t asPathLength, PATH_LIST* asPathList, AS_TYPE asType, uint32_t localAS, bool bBigEndian)
{
  uint32_t pathId=0;
  char* strBuf = NULL;
  int idx;
  ...
  printf("Debug: GeneratePathID:\n");

  int strSize = asPathLength * 4 *2 + 1 + 4 + 1;  //  Path length * 4 byte, *2: hex string, 1: AS type, 4: localAS, 1: NULL
  //int strSize = asPathLength * 4 *2 + 1 + 1;  //  Path length * 4 byte, *2: hex string, 1: AS type, 4: localAS, 1: NULL
  ...
  {
    printf("asPathList[%u]: %u\n", idx, ntohl(asPathList[idx]));
    if(bBigEndian){
      sprintf(strBuf + (idx*4*2), "%08X", ntohl(asPathList[idx]));
      printf("Debug: BigEndian\n");
    }
    else{
      sprintf(strBuf + (idx*4*2), "%08X", asPathList[idx]);
      printf("Debug: LittleEndian\n");
    }
  }
  //sprintf(strBuf+strSize-2, "%X", asType);
  sprintf(strBuf+strSize-2-4, "%X", asType);
  sprintf(strBuf+strSize-4, "%X", localAS);

  pathId = crc32((uint8_t*)strBuf, strSize);
  LOG(LEVEL_INFO, "PathID: %08X strings: %s", pathId, strBuf);
  printf("Debug: PathID: %08X strings: %s", pathId, strBuf);
