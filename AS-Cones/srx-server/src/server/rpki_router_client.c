  ...
  while (keepGoing && !client->stop)
  {
    ...
    switch (hdr->type)
    {
      ...
      case PDU_TYPE_ASPA :
        if (client->version > 1)
        {
          printf("Debug: handleReceiveAspaPdu\n");
        ...
        break;
      case PDU_TYPE_ASCONE :
        printf("Debug: handleReceiveAsconePdu NO VERSION\n");
        if (client->version > 1)
        {
          printf("Debug: handleReceiveAsconePdu\n");
          LOG(LEVEL_INFO, FILE_LINE_INFO "ASCONE PDU received from Rpki rtr server");
          // ASCONE validation  
          handleReceiveAsconePdu(client, (RPKIASCONEHeader*)byteBuffer, pduLen);
        }
        else
        {
          *errCode = RPKI_EC_UNSUPPORTED_PDU;
...
bool handleReceiveAsconePdu(RPKIRouterClient* client, RPKIASCONEHeader* hdr, 
                          uint32_t pduLen)
{
  // 
  // figure out the numbers how many customer ASes are in the received  pdu
  //
  //     1. parsing
  //     2. memcpy for customerASNs if customerAsCount is greater than 1
  //     3. inside hdr, there might have multiple customer asns
  //
  printf("Debug: Reading header inforomation...\n");
  printf("Debug: PDU-Length: %u\n", pduLen);
  uint32_t providerAsn = ntohl(hdr->asnCone);
  uint16_t customerAsCount = ntohs(hdr->customerCount);
  uint16_t ascone_pdu_type = ntohs(hdr->objectType);
  asConePduEntry *customerAsns;
  //uint8_t  flags = hdr->coneUniqueName;

  uint8_t announce       = 0x01;//flags & 0x01; // bit 0: 1 == announce, 0 == withdraw
  uint8_t addrFamilyType = 0x02;//flags & 0x02; // bit 1: AFI (IPv4 == 0, IPv6 == 1)

  uint8_t *byteHdr = (uint8_t*)hdr;
  printf("Debug: byteHdr: %u\n", byteHdr);
  asConePduEntry *startp_customerAsns = byteHdr + sizeof(RPKIASCONEHeader);
  asConePduEntry *curpt_customerASN = startp_customerAsns;
  printf("Debug: curpt_customerASN: %u\n", curpt_customerASN);
  
  // Index counter for loops
  int idx = 0;

  printf("Debug: Allocating memory for CustomerASNs(Count: %d)...\n", customerAsCount);
  customerAsns = (asConePduEntry*)calloc(customerAsCount, sizeof(asConePduEntry));

  LOG(LEVEL_INFO, "---" FILE_LINE_INFO " receive ASCONE Object PDU from rpki cache ---");
  LOG(LEVEL_INFO, "providerer asn: %d", providerAsn);
  LOG(LEVEL_INFO, "customer as count: %d", customerAsCount);

  printf("Debug: Received Provider ASN: %d", providerAsn);
  printf("Debug: Received Customer AS counter: %d", customerAsCount);

  customerAsns = startp_customerAsns;

  // 3. inside hdr, there might have multiple customer ASNs
  for (idx = 0; idx < customerAsCount; idx++,curpt_customerASN++, customerAsns++)
  {
    LOG(LEVEL_INFO, "customer asn[%d]: %d", idx, ntohl(curpt_customerASN->ASN));
    customerAsns->ASN = ntohl(customerAsns->ASN);
    //@TODO: Muss auch gesetzt werden wird bisher ignoriert.
    printf("Debug: OtherASCone wird bisher nicht gesetzt!\n");
    //customerAsns->OtherASCone = ......
    customerAsns->verified = ntohs(customerAsns->verified);

    printf("Debug: curpt_customerASN: %u\n", curpt_customerASN);
    printf("Debug: Received Customer AS counter: %d\n", idx);
    printf("Debug: Received customer asn: %d\n", ntohl(curpt_customerASN->ASN));
    printf("Debug: Received customer asn->verified: %d\n", curpt_customerASN->verified);
  }
  ...
  // this calls 'handleAsconePdu()' in rpki_handler module
  printf("Debug: Client correctly set?\n");
  printf("Debug: Client: %d \n", client);
  printf("Debug: Client->user: %d \n", client->user);
  client->params->cbHandleAsconePdu(client->user, providerAsn, customerAsCount, 
                                  startp_customerAsns, addrFamilyType, announce, ascone_pdu_type); 
  //return true;
