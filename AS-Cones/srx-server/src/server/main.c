...
static ASCONE_DBManager asconeDBManager;
static ASCONE_DBManager aspolicyDBManager;
...
static bool setupCaches()
{
  ...
  initializeAsconeDBManager(&asconeDBManager, &config);    // ASCONE: ASCONE object DB
  initializeAsconeDBManager(&aspolicyDBManager, &config);  // ASCONE: ASPOLICY object DB
  createAspathCache(&aspathCache, &aspaDBManager, &asconeDBManager, &aspolicyDBManager); // ASPA/ASCONE: AS path DB 
...
static bool setupHandlers()
{
  ...
  if (!createRPKIHandler (&rpkiHandler, &prefixCache, &aspathCache, &aspaDBManager, &asconeDBManager, &aspolicyDBManager,
