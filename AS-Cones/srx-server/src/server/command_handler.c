...
/**
 * This function performs the ASCONE validation.
 * 
 * @param asPathList
 * @param length
 * @param asType
 * @param direction
 * @param localAS
 * @param asconeDBManager
 * 
 * @return the ASCONE validation result
 */
uint8_t validateASCONE (PATH_LIST* asPathList, uint8_t length, AS_TYPE asType, 
                      AS_REL_DIR direction, uint32_t localAS,
                      ASCONE_DBManager* asconeDBManager, ASCONE_DBManager* aspolicyDBManager)
{

  //@TODO: The ValidationType should be choosen in the configuration file
  printf("Debug: ValidationType is static set to STRICT and schould be implemented into the configuration.");
  ASCONE_ValidationType asconeValType = ASCONE_VALTYPE_STRICT;

  printf("XXXXXXXXXX\n\nASCONE Validation:\n\nXXXXXXXXXX\n\n");
  printf("Path to validate:\n");
  printf("localAS: %u\n", localAS);
  //int idx;

  for (int idx = 0; idx < length; idx++)
  {
    printf("asPathList[%d]: %u\n", idx, asPathList[idx]);
  }

  ASCONE_ValidationResult tmpAsconeResult = ASCONE_RESULT_UNDEFINED;
  ASCONE_ValidationResult tmpPolicyResult = ASCONE_RESULT_UNDEFINED;
  uint16_t ascone_result_flags = 0;


  bool isDownstream, isUpstream, peerCheck;

  // Initialize the result list
  /*for (int idx = 0; idx < length; idx++)
  {
    ascone_result_list[idx] = ASCONE_RESULT_UNDEFINED;
  }*/

  // First check if the path is a downstream or upstream path.
  // If both is valid, downstream is prefered!
  uint32_t providerAS, customerAS;  

  providerAS = localAS;
  customerAS = asPathList[0];
  tmpAsconeResult = ASCONE_DB_lookup(asconeDBManager,customerAS, providerAS);

  providerAS = localAS;
  customerAS = asPathList[0];
  tmpPolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
  
  isDownstream = false;
  isUpstream = false;
  peerCheck = false;
  int number_of_upstream_provider= 0;
  int number_of_downstreamprovider = 0;
  //ASCONE_ValidationResult asconeResultList[length];
  //ASCONE_ValidationResult aspolicyResultList[length];


  if (tmpAsconeResult == ASCONE_RESULT_VALID)
  {
    isDownstream = true;
  }

  if (tmpPolicyResult == ASCONE_RESULT_VALID)
  {
    isUpstream = true;
    // Check if this is a peer
    uint8_t peer2_AspolicyResult = ASCONE_DB_lookup(aspolicyDBManager, providerAS, customerAS);
    //uint8_t peer2_AspolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
         
    if (peer2_AspolicyResult == ASCONE_RESULT_VALID)
    {
      // peer was detected
      peerCheck = true;
    }
  ...
  printf("Debug: ASCONE Result asPathList[%d]: %u, isDownstream: %u, isUpstream: %u\n", -1, tmpAsconeResult, isDownstream, isUpstream);

  if (!isDownstream && !isUpstream)
  {
    // The path includes the localAS but there is no POLICY nor an ASCONE Object. This can be a route leak.
    printf("Debug: NO UPSTREAM and NO DOWNSTREAM is valid!!\n");
    return SRx_RESULT_INVALID;
  }

  //Check asPathList as a path coming from downstream (a customer) -> we are a upstream provider 
  // (Path startpoint is AS 11 we are PEERING A or AS10)
  //
  //             PEERING A ----- PEERING B
  //                |               | 
  //               AS10            AS00
  ...
  if (isDownstream)
  {
    for(int i = length-2; i >= 0; i--){
      providerAS = asPathList[i];
      customerAS = asPathList[i+1];
      tmpAsconeResult = ASCONE_DB_lookup(asconeDBManager, customerAS, providerAS);
      printf("Debug: validateASCONE - Provider: %u, Customer: %u, Result: %u DOWNSTREAM [%u]\n", providerAS, customerAS, tmpAsconeResult, i);
      if (tmpAsconeResult == ASCONE_RESULT_INVALID)
      {
        // ASCONE Object found with no customer for the given ASN
        printf("Debug: SRX_RESULT_INVALID -> downstream path is invalid");
        return SRx_RESULT_INVALID;
      }

      // Check Policy object if AS-Cone object is there
      if (tmpAsconeResult == ASCONE_RESULT_VALID || tmpAsconeResult == ASCONE_RESULT_UNKNOWN)
      {
        providerAS = asPathList[i+1];
        customerAS = asPathList[i];
        tmpPolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
        if (tmpPolicyResult == ASCONE_RESULT_INVALID)
        {
          // Upstream element of the downstream path does not match
          printf("Debug: SRX_RESULT_INVALID -> upstream object of downstream path is invalid");
          return SRx_RESULT_INVALID;
        }
  ...
  //Check asPathList as a path coming from upstream or peering partner -> we are a downstream provider 
  // (Path startpoint is AS 11 we are PEERING B, AS00 or AS01)
  //
  //             PEERING A ----- PEERING B
  //                |               | 
  //               AS10            AS00
  ...
  if (isUpstream){
      ...
      // Find out the number of upstream or peering AS in the path
      for (int i = 0; i < length-1; i++)
      {
        providerAS = asPathList[i];
        customerAS = asPathList[i+1];
        tmpPolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
        printf("Provider: %u, Customer: %u, peerCheck: %u UPSTREAM CHECK [%u]\n", providerAS, customerAS, peerCheck, i);
        providerAS = asPathList[i+1];
        customerAS = asPathList[i];
        tmpAsconeResult = ASCONE_DB_lookup(asconeDBManager, customerAS, providerAS);
        
        //aspolicyResultList[i] = tmpPolicyResult;
        if ((tmpPolicyResult == ASCONE_RESULT_VALID && tmpAsconeResult != ASCONE_RESULT_INVALID) || (tmpPolicyResult == ASCONE_RESULT_UNKNOWN && tmpAsconeResult == ASCONE_RESULT_UNKNOWN))
        {
          // Check if this is an upstream path and a peer was detected berfore
          if (peerCheck == true)
          {
            if (tmpAsconeResult == ASCONE_RESULT_VALID)
            {
              // upstream path detected after a peer only downstream is valid
              return SRx_RESULT_INVALID;
            }
           
          }
          
          // Check for peers on the route
          uint8_t peer2_AspolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
          //uint8_t peer2_AspolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
          ...
          // An policy object is found or no policyobject at all is found for the given ASN
          number_of_upstream_provider++;
          continue;
        }
        else{
          // An invalid upstream was found, this must be the beginning of the downstream path

          // There was a policy object found, thats why there MUST exist an as-cone object
          providerAS = asPathList[i];
          customerAS = asPathList[i+1];
          tmpAsconeResult = ASCONE_DB_lookup(asconeDBManager, customerAS, providerAS);
          if (tmpAsconeResult == ASCONE_RESULT_INVALID)
          {
            // There is no valid upstream and no valid downstream
            printf("Provider: %u, Customer: %u DOWNSTREAM CHECK\n", providerAS, customerAS);
            printf("Debug: No valid upstream and no valid downstream! --> INVALID\n"); 
            return SRx_RESULT_INVALID;
          }

          // If the end of the part is not reached yet,
          // this part of the path MUST be the downstream part
          for (int j = i; j < length-1; j++)
          {
            providerAS = asPathList[j];
            customerAS = asPathList[j+1];
            tmpAsconeResult = ASCONE_DB_lookup(asconeDBManager, customerAS, providerAS);
            printf("Provider: %u, Customer: %u DOWNSTREAM CHECK [%u]\n", providerAS, customerAS, j);
            if (tmpAsconeResult == ASCONE_RESULT_INVALID)
            {
              // Downstream path has an invalid element
              return SRx_RESULT_INVALID;
            }

            // Check correct upstream as there can exist a policy object
            if (j<length-1)
            {
              providerAS = asPathList[j+1];
              customerAS = asPathList[j];
              tmpPolicyResult = ASCONE_DB_lookup(aspolicyDBManager, customerAS, providerAS);
              if (tmpPolicyResult == ASCONE_RESULT_INVALID)
              {
                printf("Debug: SRX_RESULT_INVALID -> upstream object does not match for downstream path");
                return SRx_RESULT_INVALID;
              }
  ...
  // No upstream and downstream path is valid
  printf("Debug: validateASCONE Result: INVALID - LAST\n");
  return SRx_RESULT_INVALID;
                                     ...
                                     CommandQueueItem* item)
{
  ...
  bool asconeVal = _isSet(bhdr->flags, SRX_PROXY_FLAGS_VERIFY_ASCONE);
  SRxUpdateID updateID = (SRxUpdateID)item->dataID;

  if (!originVal && !pathVal && !aspaVal && !asconeVal)
  {
  ...
  printf("\nDebug: command_handler.c - getUpdateresult - srxRes.asconeResult: %u, pathId: %08X\n", srxRes.asconeResult, pathId);
  ...
  srxRes_mod.asconeResult   = SRx_RESULT_DONOTUSE; // Indicates this
  printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
  printf("\n\n\n\nDebug: 1. srxRes_mod.aspaResult: %u srxRes_mod.asconeResult %u\n\n\n\n", srxRes_mod.aspaResult, srxRes_mod.asconeResult);
  printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
  ...
  if (originVal && (srxRes.roaResult == SRx_RESULT_UNDEFINED))
  {
    ...
    printf("Debug: origin Validation Requested!!");
                  ...
                  // of Direct neighbor check or any other reason.
  {
    ...
    if (aspl)
    {
      ...
      if (valASPAResult != aspl->aspaValResult)
      {
        ...
        aspl->aspaValResult = valASPAResult;
      ...
      printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
      printf("\n\n\n\nDebug: 2. srxRes_mod.aspaResult: %u srxRes_mod.asconeResult %u\n\n\n\n", srxRes_mod.aspaResult, srxRes_mod.asconeResult);
      printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
...
//
  // ASCONE validation
  //
  if (asconeVal && (srxRes.asconeResult == SRx_RESULT_UNDEFINED) 
              && (defRes.result.asconeResult != SRx_RESULT_INVALID))
                  // if defRes result aspaResult is invalid, this means that a 
                  // client router put this result in due to the failure 
                  // of Direct neighbor check or any other reason.
  {
    ...
    // 
    // 1. fetch validation task for Aspath from AspathCache 
    // 2. relate this job to ASCONE object DB
    // 3. validation work
    // 4. notification
    //
    // ----------------------------------------------------------------
    ...
    ASCONE_DBManager* asconeDBManager = handler->asconeDBManager;
    ASCONE_DBManager* aspolicyDBManager = handler->aspolicyDBManager;
    ASCONE_TrieNode *ASCONEroot = asconeDBManager->tableRoot;
    ...
    if (aspl)
    {
      ...
      uint8_t valASCONEResult = validateASCONE (aspl->asPathList, 
          aspl->asPathLength, aspl->asType, aspl->asRelDir, aspl->localAS, asconeDBManager, aspolicyDBManager);

      LOG(LEVEL_INFO, FILE_LINE_INFO "\033[92m"" Validation Result: %d "
          "(0:v, 2:Iv, 3:Ud 4:DNU 5:Uk, 6:Uf)""\033[0m", valASCONEResult);

      // modify Aspath Cache with the validation result
      //
      printf("Debug: Check if val Result is unequal aspl Result valASCONEResult: %u, aspl->asconeValResult: %u \n", valASCONEResult, aspl->asconeValResult);
      if (valASCONEResult != aspl->asconeValResult)
      {
        time_t cTime = time(NULL);
        aspl->lastModified = cTime;
        modifyAsconeValidationResultToAspathCache (cmdHandler->aspathCache, pathId, 
            valASCONEResult, aspl);

        // modify aspl and srx Res
        aspl->asconeValResult = valASCONEResult;
        printf("Debug: valASCONEResult: %u, aspl->asconeValResult: %u \n", valASCONEResult, aspl->asconeValResult);
      }

      // modify Update Cache
      srxRes_mod.asconeResult = aspl->asconeValResult;
      printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
      printf("\n\n\n\nDebug: 2. srxRes_mod.aspaResult: %u srxRes_mod.asconeResult %u\n\n\n\n", srxRes_mod.aspaResult, srxRes_mod.asconeResult);
      printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
    ...
    {
      LOG(LEVEL_INFO, "Unverifiable enabled for the aspath list which only includes AS_SET");
      srxRes_mod.asconeResult = SRx_RESULT_UNVERIFIABLE;  
    }
    else
    {
      LOG(LEVEL_WARNING, "Something went wrong... path list was not registered");
  ...
  // Invalid case
  else if (asconeVal && (srxRes.asconeResult == SRx_RESULT_UNDEFINED) 
              && (defRes.result.asconeResult == SRx_RESULT_INVALID)
              && (defRes.resSourceASCONE = SRxRS_ROUTER))
  {
    LOG(LEVEL_INFO, "default result from a router is invalid, so sening it as well");
    srxRes_mod.asconeResult = SRx_RESULT_INVALID;  
  }
  ...
  {
    LOG(LEVEL_INFO, "path id and srx result already exists in UpdateCache");
    srxRes_mod.asconeResult = srxRes.asconeResult; // srx Res came from UpdateCache (cEntry)
    printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
  printf("\n\n\n\nDebug: 3. srxRes_mod.aspaResult: %u srxRes_mod.asconeResult %u\n\n\n\n", srxRes_mod.aspaResult, srxRes_mod.asconeResult);
  printf("\n\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
  }
  ...
  if (   (srxRes_mod.bgpsecResult != SRx_RESULT_DONOTUSE)
      ...
      || (srxRes_mod.asconeResult != SRx_RESULT_DONOTUSE))
  {
                            ...
                            false))
    {
      RAISE_SYS_ERROR("A validation result for a non existing update [0x%08X]!",
                      updateID);
      printf("A validation result for a non existing update [0x%08X]!", updateID);
    }    
  }
...
bool broadcastResult(CommandHandler* self, SRxValidationResult* valResult)
{
  ...
  if (clientCt > 0)
  {
    ...
    pdu->resultType   = (valResult->valType & SRX_FLAG_ROA_BGPSEC_ASPA_ASCONE);
    ...
    pdu->asconeResult   = valResult->valResult.asconeResult;
    printf("Debug: command_handler.c - updateID: %08X - broadcastResult: %u, valResult->valType: %u\n", valResult->updateID, valResult->valResult.asconeResult, valResult->valType);
