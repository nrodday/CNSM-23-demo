...
int handleAsconePdu(void* rpkiHandler, uint32_t providerAsn, 
                  uint16_t customerAsCount, asConePduEntry* customerAsns, 
                  uint16_t asconeType, uint8_t announce, uint16_t ascone_pdu_type);
...
bool createRPKIHandler (RPKIHandler* handler, PrefixCache* prefixCache,
                        AspathCache* aspathCache, ASPA_DBManager* aspaDBManager, ASCONE_DBManager* asconeDBManager, ASCONE_DBManager* aspolicyDBManager,
                       const char* serverHost, int serverPort, int rpki_version)
{
  ...
  handler->asconeDBManager = asconeDBManager;
  handler->aspolicyDBManager = aspolicyDBManager;
  ...
  handler->rrclParams.cbHandleAsconePdu    = handleAsconePdu;
                             ...
                             void* rpkiHandler)
{
  ...
  printf("Debug: rpki_handler.c - Received an end of data, process RPKI Queue:\n");

  //@TODO: Reorganize the update queue process
  process_ASPA_EndOfData(uCache, handler->aspaDBManager->cbProcessEndOfData, handler);
  process_ASCONE_EndOfData(uCache, handler->asconeDBManager->cbProcessEndOfData, handler);

  printf("Debug: Check if EndOfData call is needed for aspolicyDBManager!\n");
  // Check if this call is needed here, asconeDBManager should already take care of it.
  //process_ASCONE_EndOfData(uCache, handler->aspolicyDBManager->cbProcessEndOfData, handler);
  ...
  while (rq_dequeue(rQueue, &queueElem))
  {
    ...
    valRes.valResult.asconeResult   = SRx_RESULT_DONOTUSE;
    ...
    printf("Debug: queueElem.reason: %u\n", queueElem.reason);
    ...
    if ((queueElem.reason & RQ_ASPA) == RQ_ASPA)
    {
      ...
      printf(" called for ASPA dequeue [uID: %08X] \n", *uID);
      uint32_t pathId= 0;
      if (getUpdateResult(uCache, uID, 0, NULL, &srxRes, &defaultRes, &pathId))
      {
        printf("\n\n\n\n\n\nDebug: WARNING WARNING WARNING ASPA RESULT TYPE IS CHANGED!!!!!\n\n\n\n\n");
    ...
    // Here check for ASCONE Validation which was registered 
    if ((queueElem.reason & RQ_ASCONE) == RQ_ASCONE)
    {
      LOG(LEVEL_INFO, FILE_LINE_INFO " called for ASCONE dequeue [uID: %08X] ", *uID);
      uint32_t pathId= 0;
      if (getUpdateResult(uCache, uID, 0, NULL, &srxRes, &defaultRes, &pathId))
      {
         printf("\n\n\n\n\n\nDebug: WARNING WARNING WARNING ASCONE RESULT TYPE IS CHANGED!!!!!\n\n\n\n\n");
        valRes.valType |= VRT_ASCONE;
        valRes.valResult.asconeResult = srxRes.asconeResult;
      }
      else
      {
        LOG(LEVEL_WARNING, "Update 0x%08X not found during de-queuing of RPKI "
...
//  
  printf("Debug: ASPA HANDLER IS CALLED!!!\n");
...
/**
 * Process the ASCONE PDU received from the Validation Cache and store the data
 * in the ASCONE database.
 * 
 * @param rpkiHandler      Pointer to the RPKI Handler itself.
 * @param providerAsn      The ASN of the provider
 * @param customerAsCount  Number of customers in the customersAsns list.
 * @param customererAsns     The list of customer ASNs as struct
 * @param asconeType       Is this object an AS-CONE or an AS-POLICY object?
 * @param announce         Indicates if this in an announcement or not.
 * 
 * @return 1 for success, 0 for failure.
 * 
 * @since 0.6.0.0 - ASCONE
 */

int handleAsconePdu(void* rpkiHandler, uint32_t providerAsn, 
                  uint16_t customerAsCount, asConePduEntry* customerAsns, 
                  uint16_t asconeAddrFamType, uint8_t announce, uint16_t ascone_pdu_type)
{
// 
// ASCONE validation (called from params -> cbHandleAsconePdu in handleReceiveAsconePdu function)
// work1. parsing ASCONE objects into AS number in forms of string
// work2. call DB to store
//  
  printf("Debug: ASCONE HANDLER IS CALLED!!!\n");
  LOG(LEVEL_INFO, FILE_LINE_INFO " ASCONE handler called for registering ASCONE object(s) into DB");

  if (ascone_pdu_type != ASCONE_OT_ASCONE)
  {
    printf("Debug: Policy OBject detected. Changing DBManager!!!\n");
    printf("Debug: ascone_pdu_type: %u\n", ascone_pdu_type);
    //return 0;
  }
  ...
  // If the object Type is a ASCONE-Policy switch DB Handler
  if (ascone_pdu_type == ASCONE_OT_ASPOLICY)
  {
    asconeDBManager = handler->aspolicyDBManager;
  }
  
  
  ASCONE_TrieNode *node = NULL;
  ASCONE_Object *asconeObj = NULL;
  //@TODO: Implement the string for naming ascones!!!
  printf("Debug: Implement STring for ASCONE Objects");
  asconeObj = newASCONEObject(providerAsn, NULL, asconeAddrFamType, customerAsCount, customerAsns);

  char strWord[6];
  sprintf(strWord, "%d", providerAsn);
  

  if (announce == 1) // 1 == announce, 0 == withdraw
  {
    LOG(LEVEL_INFO, "[Announce] ASCONE/POLICY object, search key in DB: %s", strWord);
    node = insertAsconeObj(asconeDBManager, strWord, strWord, asconeObj);
    if (node)
    {
      retVal = 1; // success
    }
  ...
  {
    LOG(LEVEL_INFO, "[Withdraw] ASCONE object, search key in DB: %s", strWord);
    bool resWithdraw = delete_TrieNode_AsconeObj (asconeDBManager, strWord, asconeObj);
    ...
    if (asconeObj) // release memory unless used for inserting db
    {
      if (asconeObj->customerAsns)
      {
        free(asconeObj->customerAsns);
      }
      free (asconeObj);
      asconeObj = NULL;
    }
