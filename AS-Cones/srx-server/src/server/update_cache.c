                     ...
                     SRxResult* srxRes, SRxDefaultResult* defaultRes, uint32_t *pathId)
{
  ...
  if (tableFind(self, updID, &cEntry))
  {
    ...
    srxRes->asconeResult            = cEntry->srxResult.asconeResult;
    defaultRes->resSourceASCONE     = cEntry->defaultResult.resSourceASCONE;
    defaultRes->result.asconeResult = cEntry->defaultResult.result.asconeResult;
  ...
  else
  {
    ...
    // ASCONE Validation Values
    srxRes->asconeResult            = SRx_RESULT_UNDEFINED;
    defaultRes->resSourceASCONE     = SRxRS_DONOTUSE;
    defaultRes->result.asconeResult = SRx_RESULT_DONOTUSE;
  }
                ...
                SRxDefaultResult* defRes, BGPSecData* bgpData, uint32_t pathId)
{
  ...
  LOG(LEVEL_DEBUG, HDR "Store update [ID:0x%08X] in update cache.",
                   pthread_self(), updID);
  printf("Debug: update_cache.c - Store update [ID:0x%08X] in update cache. pathID: %08X", updateID, pathId);
  ...
  //    unlockMutex(&self->itemMutex);
    ...
    cEntry->srxResult.asconeResult = SRx_RESULT_UNDEFINED;
    ...
    if (defRes != NULL)
    {
      ...
      cEntry->defaultResult.resSourceASCONE     = defRes->resSourceASCONE;
    }
    else
    {
      ...
      cEntry->defaultResult.resSourceASCONE     = SRxRS_UNKNOWN;
    }
                        ...
                        SRxResult* result, bool suppressNotification)
{
  ...
  else
  {
    ...
    valRes.valResult.asconeResult = cEntry->srxResult.asconeResult;
    ...
    {       
      printf("Debug: modifyUpdateResult - ASPA CALLED THIS SHOULD BE - DONT USE MAYBE AND IS A BUG....!!!\n");
      printf("Debug: valRes.valType: %u, result->aspaResult: %u\n", valRes.valType,result->aspaResult);
      printf("Debug: valRes.valType: %u, result->asconeResult: %u\n", valRes.valType,result->asconeResult);
    ...
    // Check if ASCONE results can be used.
    if (result->asconeResult != SRx_RESULT_DONOTUSE)
    { 
      valRes.valType |= VRT_ASCONE;
      valRes.valResult.asconeResult = result->asconeResult;

      printf("Debug: result->asconeResult: %u, cEntry->srxResult.asconeResult: %u, valType: %u SHOULD BE 8", result->asconeResult, cEntry->srxResult.asconeResult, valRes.valType);

      if (result->asconeResult != cEntry->srxResult.asconeResult)
      {
        cEntry->srxResult.asconeResult = result->asconeResult;
      }
    }
                        ...
                        SRxResult* srxResult_ascone)
{
  ...
  if (!tableFind(self, updID, &cEntry))
  {
    RAISE_SYS_ERROR("Does not exist in update cache, can not modify ascone result!");
    retVal = false;
  }
  else
  {
    ...
    // Check if ASCONE srxResult_ascone can be used.
    if (srxResult_ascone->asconeResult != SRx_RESULT_DONOTUSE)
    { // Check for changes in bgpsec srxResult_ascone
      if (srxResult_ascone->asconeResult != cEntry->srxResult.asconeResult)
      {
        cEntry->srxResult.asconeResult = srxResult_ascone->asconeResult;
        printf("\nDebug: ASCONE \033[92m""cEntry(UpdateCache) updated with uID: %08X, ASCONE result:%d ""\033[0m\n", 
            *updateID, srxResult_ascone->asconeResult);
        LOG(LEVEL_INFO, "\nASCONE \033[92m""cEntry(UpdateCache) updated with uID: %08X, ASCONE result:%d ""\033[0m\n", 
            *updateID, srxResult_ascone->asconeResult);
        retVal = true;
      }
    }
                            ...
                            void* rpkiHandler)
{
  ...
  HASH_ITER(hh, (CacheEntry*)self->table, cEntry, tmp) 
  {
    ...
    printf("Debug: update_cache.c - ITER - updateID: %08X, aspathCacheID: %08X\n",cEntry->updateID,cEntry->aspathCacheID);
    cb((void*)self, (void*)rpkiHandler, cEntry->updateID, cEntry->aspathCacheID, 
        lastEndOfDataTime); // call process_ASCONE_EndOfData_main
  }
