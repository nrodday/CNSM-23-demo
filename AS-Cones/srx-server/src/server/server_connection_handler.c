                              ...
                              SRXRPOXY_BasicHeader_VerifyRequest* hdr)
{
  ...
  printf("Debug: Enter processValidationRequest\n", pthread_self());
  ...
  bool     doAsconeVal   = (hdr->flags & SRX_FLAG_ASCONE) == SRX_FLAG_ASCONE;

  printf("Debug: server_connection_handler.c- SHOULD BE 1 if 0 the SRX Flag is not correctly handled! - doAsconeVal: %u\n", doAsconeVal);
  printf("Debug: doAsconeVal: %u\nhdr->flags:%u\nSRX_FLAG_ASCONE: %u\n", doAsconeVal, hdr->flags, SRX_FLAG_ASCONE);
...
//                                        : sizeof(SRXPROXY_VERIFY_V6_REQUEST));
  ...
  uint8_t clientID = (doOriginVal || doPathVal || doAspaVal || doAsconeVal) ? client->routerID : 0;
  printf("Debug: server_connection_handler.c - processValidationRequest - clientID: %u\n", clientID);
  ...
  if (v4)
  {
    ...
    printf("  prefix->ip.addr.v4: %u.%u.%u.%u, originAS: %u\n", prefix->ip.addr.v4.u8[0], prefix->ip.addr.v4.u8[1], prefix->ip.addr.v4.u8[2], prefix->ip.addr.v4.u8[3], originAS);
  }
  else
  {
  ...
  printf("\nYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY\n\n\n\n\n\n Debug: originAS: %u\nlocal-as: %u updateID: %08X\n\n\n\n\n\n", originAS, ntohl(bgpData.local_as), updateID);
  ...
  printf("Debug: updateID: %08X", updateID);
  ...
  printf("Debug: server_connection_handler.c -  srxRes.asconeResult: ",srxRes.asconeResult);
  ...
  SRxResult srxRes_ascone;
  bool modifyUpdateCacheWithAspaValue = false;
  bool modifyUpdateCacheWithAsconeValue = false;
  ...
  { 
    ...
    pathId = makePathId(bgpData.numberHops, bgpData.asPath, asType, ntohl(bgpData.local_as),true);
    LOG(LEVEL_INFO, FILE_LINE_INFO " generated Path ID : %08X ", pathId);
    printf("Debug: generated Path ID : %08X \n", pathId);
    printf("Debug: processValidationRequest - server_connection_handler - THIS PATH CHECK MUST BE MADE ASCONE READY!!!!\n");

    // to see if there is already exist or not in AS path Cache with path id
    aspl = getAspathListFromAspathCache (self->aspathCache, pathId, &srxRes);
    ...
    if(aspl)
    {
      ...
      if (srxRes.aspaResult == SRx_RESULT_UNDEFINED)
      {
      ...
      else
      {
        ...
        // not undefined
        LOG(LEVEL_INFO, FILE_LINE_INFO " ASPA validation Result[%d] is already exist", srxRes.aspaResult);
       ...
       srxRes_aspa.aspaResult = srxRes.aspaResult;


      // @TODO: Just copied from ASPA.
      // once found ascone result value in Cache, no need to validate operation
      //  this value is some value not undefined
      if (srxRes.asconeResult == SRx_RESULT_UNDEFINED)
      {
        printf("Debug: server_connection_handler ASCONE Already registered with the previous pdu");
        LOG(LEVEL_INFO, FILE_LINE_INFO " Already registered with the previous pdu");
      }
      else
      {
        ...
        // stores srx result value in db with the matched path id. So srxRes_aspa.aspaResult is
        // not undefined
        LOG(LEVEL_INFO, FILE_LINE_INFO " ASCONE validation Result[%d] is already exist", srxRes_ascone.asconeResult);
        printf("Debug:  ASCONE validation Result[%u] is already exist\n", srxRes_ascone.asconeResult);

        // Modify UpdateCache's srx Res -> aspaResult with srxRes_ascone.asconeResult
        // But UpdateCache's cEntry here dosen't exist yet
        // so, after calling storeUpdate, put this value into cEntry directly
        modifyUpdateCacheWithAsconeValue = true;
      }
      printf("Debug: srxRes.asconeResult: %u\n", srxRes.asconeResult);
      srxRes_ascone.asconeResult = srxRes.asconeResult;
      printf("Debug: srxRes_ascone.asconeResult: %u\n", srxRes_ascone.asconeResult);
      printf("Debug: modifyUpdateCacheWithAspaValue: %u, modifyUpdateCacheWithAsconeValue: %u\n", modifyUpdateCacheWithAspaValue,modifyUpdateCacheWithAsconeValue);

    }
    // AS Path List not exist in Cache
    else
    {
      printf("Debug: SRX Create NEW entry for ASPATH! - bgpData.local_as: %u\n", ntohl(bgpData.local_as));
      aspl = newAspathListEntry(bgpData.numberHops, bgpData.asPath, pathId, asType, asRelDir, bgpData.afi, bgpData.local_as, true);
      if(!aspl)
      {
      ...
      if (doStoreUpdate)
      {
        ...
        //defResInfo.result.asconeResult = SRx_RESULT_UNDEFINED;//hdr->asconeDefRes; // router's input value (Undefined, Unverifiable, Invalid)
        defResInfo.result.asconeResult = hdr->asconeDefRes; // router's input value (Undefined, Unverifiable, Invalid)
        defResInfo.resSourceASCONE     = hdr->asconeResSrc;
      }
      ...
      srxRes.asconeResult   = defResInfo.result.asconeResult;
  ...
  if (modifyUpdateCacheWithAsconeValue)
  {
    // modify UpdateCache with srxRes_aspa.aspaResult, then later this value 
    // in UpdateCahe will be used
    //@TODO: Must be made ASCONE Ready
    printf("Debug: THIS MUST BE MADE ASCONE READY!!! srxRes_ascone.asconeResult: %u\n", srxRes_ascone.asconeResult); 
    modifyUpdateCacheResultWithAsconeVal(self->updateCache, &updateID, &srxRes_ascone);
  ...
  if (sendFlags > 0) // a notification is needed. flags specifies the type
  {
    ...
    if (doAsconeVal)
    {
      sendFlags = sendFlags | SRX_FLAG_ASCONE;
    }
    

    printf("Debug: SRX server_connection_handler - doAsconeVal: %u sendFlags: %u\n", doAsconeVal, sendFlags);


    printf("Debug: SRX server_connection_handler - doAspaVal: %u sendFlags: %u\n", doAspaVal, sendFlags);
    ...
    if (!sendVerifyNotification(svrSock, client, updateID, sendFlags,
                                ...
                                srxRes.bgpsecResult, srxRes.aspaResult, srxRes.asconeResult,
  ...
  if ((doOriginVal || doPathVal || doAspaVal || doAsconeVal) && ((sendFlags & SRX_FLAG_ROA_BGPSEC_ASPA_ASCONE) > 0))
  {
    // Only keep the validation flags.
    printf("Debug: SRX server_connection_handler - check if this values are correct here for ASCONE!\n");
    hdr->flags = sendFlags & SRX_FLAG_ROA_BGPSEC_ASPA_ASCONE;

    printf("Debug: server_connection_handler.c- XXXX queueCommand XXXX - updateID: %08X - doAsconeVal: %u, doAspaVal: %u\n", doAsconeVal, doAspaVal);
    printf("Debug: doAsconeVal: %u\nhdr->flags:%u\nSRX_FLAG_ASCONE: %u\n", updateID, doAsconeVal, hdr->flags, SRX_FLAG_ASCONE);

    // create the validation command!
    printf("Debug: server_connection_handler.c - try to queue command - updateID: %08X", updateID);
                         ...
                         void* srvConHandler)
{
  ...
  else
  {
    ...
    //printf("Debug: Received Validation Request from proxy[0x%d], type=%d\n", clientThread, bhdr->type);
    printf("Debug: Received Validation Request type=%d\n", bhdr->type);
    switch (bhdr->type)
    {
      ...
      case PDU_SRXPROXY_VERIFY_V6_REQUEST:
        ...
        {
          //printf("Debug: Received Validation Request from proxy[0x%08X], type=%u\n",
          //                                pthread_self(), clientThread, bhdr->type);
          ...
          if(((((SRXRPOXY_BasicHeader_VerifyRequest*)packet)->flags & SRX_FLAG_ASPA) == SRX_FLAG_ASPA)){
            printf("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\nDebug: Received ASPA request!!!!\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
          }
