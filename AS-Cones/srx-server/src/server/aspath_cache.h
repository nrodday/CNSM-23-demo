...
typedef struct {  
  ...
  ASCONE_DBManager  *asconeDBManager;
  ASCONE_DBManager  *aspolicyDBManager;
} AspathCache;
...
AS_PATH_LIST* newAspathListEntry (uint32_t length, uint32_t* pathData, uint32_t pathId, 
                                  AS_TYPE asType, AS_REL_DIR asRelDir, uint16_t afi, uint32_t localAS, bool bBigEndian);
