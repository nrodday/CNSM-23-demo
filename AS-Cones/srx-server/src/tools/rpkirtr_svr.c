...
// ASPA/AS-CONE objects can become rather large.
...
typedef struct {
  ...
  /** The AS number for this entry / Customer AS if ASPA / Provider AS if ASCONE*/
  uint32_t  asNumber;
  /** The IP Address */
  union {
    /** The IPv4 Address */
  ...
  uint16_t  customerCount;
  /** The neighbour information */
  uint16_t  neighbourCount;
  ...
  { // Send the prefix
    ...
    else
    {
      ...
      if (cache.entries.size > 0) // there is always a root.
      {
        ...
        // This is used to allow to grow the ASCONE PDU memory as needed.
        // The ASCONE PDU size depends on the number of customerASes sent.
        // + 4 Because each ASPAHeader contains at least one ProviderAS which
        // in NOT part of the static Header structure because 1..N providers
        // van exist.
        uint32_t              asconeBufferSize    = sizeof(RPKIASCONEHeader);
        uint32_t              asconeBufferMinSize = asconeBufferSize;
        //@TODO: Remove the memory allocation from here, very bad style!
        uint8_t*              asconeBuffer = malloc(asconeBufferSize);
        RPKIASCONEHeader*       asconehdr = (RPKIASCONEHeader*)asconeBuffer;
        memset(asconeBuffer, 0, asconeBufferSize);
        ...
        // ASCONE PDU
        asconehdr->version = version;
        asconehdr->type  = PDU_TYPE_ASCONE;
        asconehdr->zero_1 = 0;
        ...
        for (; currNode; currNode = getNextNodeOfSListNode(currNode))
        {
          ...
          else if (cEntry->isASPA)
          {
            if (version > 1)
            {
              ERRORF("Debug: Sending ASPA Object!\n");
          ...
          else if (cEntry->isASCONE)
          {
            if (version > 1)
            {
              ERRORF("Debug: Building AS-Cone PDU\n");
              //printf("Press Any Key to Continue\n");  
              //getchar();
              // First determine if the header buffer is large enough.
              //@TODO: Größe des Speichers anpassen!!!!
              asconeBufferMinSize = sizeof(RPKIASCONEHeader)
                                  + (ntohs(cEntry->customerCount) * sizeof(asConePduEntry));
              
              //asconeBufferMinSize = sizeof(RPKIASCONEHeader);
              //asconehdr->type = 
              asconehdr->length            = htonl(asconeBufferMinSize);
              
              asconehdr->customerCount = cEntry->customerCount;
              asconehdr->asnCone      = cEntry->asNumber;
              asconehdr->objectType   = htons(ASCONE_OT_ASCONE);
              if (asconeBufferSize < asconeBufferMinSize)
              {
                uint8_t* newBuffer = realloc(asconeBuffer, asconeBufferMinSize);
                if (newBuffer != NULL)
                {
                  asconeBuffer     = newBuffer;
                  asconeBufferSize = asconeBufferMinSize;
                  asconehdr        = (RPKIASCONEHeader*)asconeBuffer;
                  newBuffer      = NULL;
                  printf("Debug: realloc executed!");
                }
                else
                {
                  ERRORF("Error: Not enough memory for ASCONE object!\n");
                  break;
                }
              }
              ERRORF("Debug: asconehdr->type: %d\n", asconehdr->type);
              ERRORF("Debug: asconeBufferMinSize: %d\n", asconeBufferMinSize);
              ERRORF("Debug: asconehdr->length: %d\n", ntohl(asconehdr->length));
              ERRORF("Debug: asconehdr->customerCount: %d\n", ntohs(asconehdr->customerCount));
              ERRORF("Debug: asconehdr->asnCone: %d\n", ntohl(asconehdr->asnCone));
              ERRORF("Debug: asconehdr_customer_size: %d\n", sizeof(asConePduEntry));
              // Now use a helper pointer to copy the list of customer ASs to 
              // into the PDU
              uint8_t* ptr = asconeBuffer + sizeof(RPKIASCONEHeader);
              
              printf("Debug: ptr: %u \nSize of customerAS Entries: %u\n", ptr, ntohs(cEntry->customerCount) * sizeof(asConePduEntry));
              memcpy(ptr, cEntry->providerAS,
                     ntohs(cEntry->customerCount) * sizeof(asConePduEntry));
              
              printf("Debug: ptr: %u\n", ptr);

              //asConePduEntry *ptr_debug_customer = (asConePduEntry *)cEntry->providerAS;
              //asConePduEntry *ptr_debug_customer = (asConePduEntry *)ptr;
              printf("Debug: asconehdr: %u\n asconeBuffer: %u\n", asconehdr, asconeBuffer);
              //asConePduEntry* ptr_debug_customer = asconehdr+sizeof(RPKIASCONEHeader);
              asConePduEntry* ptr_debug_customer = asconeBuffer+sizeof(RPKIASCONEHeader);
              printf("Debug: ptr_debug_customer: %u\n", ptr_debug_customer);
              int i = 0;
              for (i = 0; i < ntohs(cEntry->customerCount); i++, ptr_debug_customer++)
              {
                printf("Debug: Sending Customer [%d] ASN: %d\n", i, ntohl(ptr_debug_customer->ASN));
                printf("Debug: Sending Customer [%d] OtherASCone: %s\n", i, ptr_debug_customer->OtherASCone);
                printf("Debug: Sending Customer [%d] verified: %d\n", i, ptr_debug_customer->verified);
              }
              
              printf("Debug: Object-Size: %u\n", asconeBufferMinSize);
              ERRORF("Debug: trieing to send AS-Cone Object\n");
              // Here asconehdr is a pointer so we can hand is over directly.
              /*if (!sendNum(fdPtr, asconehdr, asconeBufferMinSize))
              {
                ERRORF("Error: Failed to send an 'ASCONE' object!\n");
                break;
               }*/
              if (!sendNum(fdPtr, asconeBuffer, asconeBufferMinSize))
              {
                ERRORF("Error: Failed to send an 'ASCONE' object!\n");
                break;
              }
          ...
          else if (cEntry->isASPOLICY)
          {
            if (version > 1)
            {
              ERRORF("Debug: Building AS-Cone Policy PDU\n");
              asconeBufferMinSize = sizeof(RPKIASCONEHeader)
                                  + (ntohs(cEntry->neighbourCount) * sizeof(asConePduEntry));
              
              //asconeBufferMinSize = sizeof(RPKIASCONEHeader);
              //asconehdr->type = 
              asconehdr->length            = htonl(asconeBufferMinSize);
              
              asconehdr->customerCount = cEntry->neighbourCount;
              asconehdr->asnCone      = cEntry->asNumber;
              asconehdr->objectType   = htons(ASCONE_OT_ASPOLICY);
              if (asconeBufferSize < asconeBufferMinSize)
              {
                uint8_t* newBuffer = realloc(asconeBuffer, asconeBufferMinSize);
                if (newBuffer != NULL)
              ...
              printf("Debug: ptr: %u \nSize of neighbourAS Entries: %u\n", ptr, ntohs(cEntry->neighbourCount) * sizeof(asConePduEntry));
              //memcpy(ptr, cEntry->providerAS,
              //       ntohs(cEntry->customerCount) * sizeof(asConePduEntry));

              
              // Generate AS-Policy Neighbour entries for the AS-CONE PDU.
              asConePduEntry* curPDUEntry = ptr;
              uint32_t* curNeighbourEntry = cEntry->providerAS;
              
              for (int i = 0; i < ntohs(cEntry->neighbourCount); i++, curPDUEntry++, curNeighbourEntry++)
              {
                curPDUEntry->ASN = *curNeighbourEntry;
                // Following entries are ignored and intentionally left blank as AS-Policy Objects only needs a neighbour ASN
                // curPDUEntry->OtherASCone
                // curPDUEntry->verified
              }
              curNeighbourEntry = NULL;
              curPDUEntry = NULL;
              ...
              printf("Debug: ptr_debug_customer: %u\n", ptr_debug_customer);
              int i = 0;
              for (i = 0; i < ntohs(cEntry->neighbourCount); i++, ptr_debug_customer++)
              {
                printf("Debug: Sending Neighbour [%d] ASN: %d\n", i, ntohl(ptr_debug_customer->ASN));
                printf("Debug: Sending Neighbour [%d] OtherASCone: %s\n", i, ptr_debug_customer->OtherASCone);
                printf("Debug: Sending Neighbour [%d] verified: %d\n", i, ptr_debug_customer->verified);
              }
        ...
        if (asconeBuffer != NULL)
        {
          printf("Debug: ASCONE Buffer free!!!!\n");
          //free (asconeBuffer);
          asconeBuffer = NULL;
        }
      }
...
//       provider, the ASPA Profile draft says 1..N providers.
    ...
    cEntry->prefixMaxLength = 0;
    cEntry->isASPA          = true;
    cEntry->isASCONE        = false;
    cEntry->isASPOLICY      = false;
...
/**
 * Read AS-CONE data from a given file or from command line. An error while
 * reading from command line will result in skipping the line and posting a
 * WARNING. An error from command line results in abort of the operation.
 *
 * @param arg The filename or the data provided via command line.
 ...
 * @return true if the prefix(es) could be send.
 */
bool readASCONEData(const char* arg, SList* dest, uint32_t serial, bool isFile)
{

  //return false; // Not completely implemented!!
...
//  #define NUM_FIELDS    3  // prefix max_len as
 ...
 struct asConeEntry
  {
    uint32_t     ASN;
    uint8_t      OtherASCone[255];
    uint8_t      verified;
  };

  //typedef struct asConeEntry asConeEntry;
  typedef asConePduEntry asConeEntry;
  asConeEntry* curASCONE = NULL;

 uint32_t       providerASN = 0;
 uint16_t       customerCount = 0;
 uint8_t        verified = 0;
 uint8_t*       customerBuff = NULL;
 asConeEntry*      customerASN = NULL;
 int            customerCounter = 0;
 char           otherASConeString[255]="NOT IMPLEMENTED YET!!!";  

  uint32_t      asnCone = 0;
  uint8_t       coneUniqueName[255];
  uint8_t       coneVersion = 0;
  time_t        coneLastModified = 0;
  time_t        coneCreated = 0;
  uint8_t       coneContactEmail[75];
  ...
  else
  {
    if (arg == NULL)
    {
      ERRORF("Error: Data missing: <provider-as> <customer-as> <verified> [(<customer-as> <verified>)*]\n");
      return false;
    }
  }
  ...
  for (; goOn; lineNo++)
  {
    ...
    ERRORF("Debug: num_fields: %d", num_fields);

    // Parse fields
    providerASN = strtoul(fields[0], NULL, 10);
    SKIP_IF(providerASN == 0, "Invalid Provider ASN", fields[0]);
    ...
    if (customerBuff)
    {
      ...
      // Check if there are at least two parameter for every entry!
      SKIP_IF(((idx-1)%2)>0, "The number of parameter is not correctly aligned.","0");
      if (idx > 1)
     {
       customerBuff = malloc((idx-1)/2 * sizeof(asConeEntry));
        //@TODO: Remove Debug Output
        ERRORF("Debug: Allocated: %d", (idx-1)/2 * sizeof(asConeEntry));
       memset(customerBuff, 0, (idx-1)/2 * sizeof(asConeEntry));
       //customerASN = (asConeEntry*)customerBuff;
       customerASN = customerBuff;
       customerCount=(idx-1)/2;
        //@TODO: Remove
       ERRORF("Debug: customerCount: %d", customerCount);
 
       SKIP_IF(customerCount==0, "At least one customer MUST be specified! Else it is a leave and do not need an AS-Cone.", "0");
       for (customerCounter = 1; customerCounter <= customerCount; customerCounter++, customerASN++)
       {
         ERRORF("Debug: Customer-ASN: %d\n", strtoul(fields[customerCounter*2-1], NULL, 10));
         customerASN->ASN = htonl(strtoul(fields[customerCounter*2-1], NULL, 10));
         
         verified = strtoul(fields[customerCounter*2], NULL, 10);
         ERRORF("Debug: verified: %d\n", verified);
         
         //verified = 1;
         SKIP_IF(!BETWEEN(verified,0,1), "Invalid verified field!", fields[customerCounter*2]);
         //customerASN->verified = htons(verified);
         customerASN->verified = verified;

         //@TODO: Insert length check for parameter.
         //strcpy(otherASConeString, "Not implemendet yet!");
         
         // Convert String(char array) into uint8_t expect that char is a typedef of uint8_t on the architecture
         strcpy(customerASN->OtherASCone, otherASConeString);  
         ERRORF("Debug: Customer %d added!", customerCounter);        
         
       }
     }
     else
     {
       //@TODO: Can this lead to a double free? when the first line has an idx <=1?
       free(customerBuff);
       customerBuff = NULL;
       customerASN   = NULL;
       SKIP_IF(false, "At lease one provider is required!", "");
     ...
     // Append AS-Cone Object to the Database
     cEntry = (ValCacheEntry*)appendToSList(dest, sizeof(ValCacheEntry));
     if (cEntry == NULL)
     {
       fclose(fh);
       if (customerBuff != NULL)
       {
         free(customerBuff);
       }
       customerBuff = NULL;
     ...
     cEntry->isASPA          = false;
     cEntry->isASPOLICY      = false;
     cEntry->isASCONE        = true;
     cEntry->asNumber        = htonl(providerASN);
     cEntry->customerCount   = htons(customerCount);
     cEntry->providerAS      = customerBuff;
     //cEntry->providerAS      = (u_int8_t *)customerASN;
     customerBuff = NULL;
     customerASN   = NULL;
     int8_t tmp_veryfied = ntohs(((asConePduEntry *)(cEntry->providerAS))->verified);
    ERRORF("Debug: cEntry->asNumber: %d", ntohl(cEntry->asNumber));
    ERRORF("Debug: cEntry->customerCount: %d", ntohs(cEntry->customerCount));
    ERRORF("Debug: cEntry->verified: %d", tmp_veryfied);
    ERRORF("Debug: AS-CONE Object is added to the cache!");
    asConePduEntry *debug_ptr = NULL;
    debug_ptr = cEntry->providerAS;
     for (idx = 0; idx < ntohs(cEntry->customerCount); idx++,debug_ptr++)
      {
        printf("Debug: cEntry Customer counter: %d\n", idx);
        printf("Debug: cEntry customer asn: %d\n", ntohl(debug_ptr->ASN));
        printf("Debug: cEntry customer verified: %d\n", debug_ptr->verified);
      }   
  }
  ...
  //ERRORF("Debug: AS-CONE Object is not added to the cache!");
  //return false; // Currently not completely implemented!!!
  return true;
...
/**
 * Read AS-POLICY data from a given file or from command line. An error while
 * reading from command line will result in skipping the line and posting a
 * WARNING. An error from command line results in abort of the operation.
 *
 * @param arg The filename or the data provided via command line.
 ...
 * @return true if the prefix(es) could be send.
 */
bool readASPOLICYData(const char* arg, SList* dest, uint32_t serial, bool isFile)
{
...
//  #define NUM_FIELDS    3  // prefix max_len as
  ...
  uint8_t*       neighbourBuff = NULL;
  uint32_t*      neighbourASN = NULL;
  ...
  for (; goOn; lineNo++)
  {
    ...
    if (neighbourBuff)
    {
      ...
      // NOT NEEDED for Policy Object Check if there are at least two parameter for every entry!
      //SKIP_IF(((idx-1)%2)>0, "The number of parameter is not correctly aligned.","0");
      if (idx > 1)
     {
       neighbourBuff = malloc((idx-1) * sizeof(u_int32_t));
        //@TODO: Remove Debug Output
        ERRORF("Debug: Allocated: %d", (idx-1) * sizeof(providerASN));
       memset(neighbourBuff, 0, (idx-1) * sizeof(providerASN));
       
       neighbourASN = neighbourBuff;
       neighbourCount=idx-1;
        //@TODO: Remove
       ERRORF("Debug: neighbourCount: %d", neighbourCount);
 
       SKIP_IF(neighbourCount==0, "At least one neighbour MUST be specified! Else there are no peering or upstream partner who need an AS-Policy.", "0");
       for (neighbourCounter = 1; neighbourCounter <= neighbourCount; neighbourCounter++, neighbourASN++)
       {
         ERRORF("Debug: Neighbour-ASN: %d\n", strtoul(fields[neighbourCounter], NULL, 10));
         *neighbourASN = htonl(strtoul(fields[neighbourCounter], NULL, 10));
     ...
     else
     {
       ...
       neighbourBuff = NULL;
       neighbourASN   = NULL;
       SKIP_IF(false, "At lease one neighbour is required!", "");
     }
     ...
     cEntry->isASPA          = false;
     cEntry->isASCONE        = false;
     cEntry->isASPOLICY      = true;
     cEntry->asNumber        = htonl(providerASN);
     cEntry->customerCount   = htons(neighbourCount);
     cEntry->neighbourCount =  htons(neighbourCount);
     cEntry->providerAS      = neighbourBuff;
     neighbourBuff = NULL;
     neighbourASN   = NULL;
    ...
    ERRORF("Debug: cEntry->customerCount: %d", ntohs(cEntry->customerCount));
    ERRORF("Debug: AS-CONE Object is added to the cache!");
    u_int32_t *debug_ptr = NULL;
    debug_ptr = cEntry->providerAS;
     for (idx = 0; idx < ntohs(cEntry->neighbourCount); idx++,debug_ptr++)
      {
        printf("Debug: cEntry neighbour counter: %d\n", idx);
        printf("Debug: cEntry neighbour asn: %d\n", ntohl(*debug_ptr));
      }   
  }
...
int showHelp(char* command)
{
  if (command == NULL)
  {
    ...
    printf("\nDisplay Commands:\n"
           ...
           "                 any delay!\n"
           "  - addASCONE <provider-as> <customer-as> <verified-bool> [(<customer-as> <verified-bool>)*]\n"
           "                 Manually add an AS-CONE object to the cache\n"
           "  - addASCONENow <provider-as> <customer-as> <verified-bool> [(<customer-as> <verified-bool>)*]\n"
           "                 Manually add an AS-CONE object to the cache\n"
           "  - addASPOLICY <provider-as> <neighbour-as> [(<neighbour-as>)*]\n"
           "                 Manually add an AS-POLICY object to the cache\n"
           "  - addASPOLICYNow <provider-as> <neighbour-as> [(<neighbour-as>)*]\n"
           "                 Manually add an AS-POLICY object to the cache\n"
 ...
 * @return true if the cache entrie(s) could be added.
 */
bool appendASCONEData(char* arg, bool fromFile)
{
  size_t  numBefore, numAdded;
  bool    succ;
  ...
  changeReadToWriteLock(&cache.lock);
  succ = readASCONEData(arg, &cache.entries, cache.maxSerial + 1, fromFile);
  changeWriteToReadLock(&cache.lock);
  ...
  OUTPUTF(false, "Read %d AS-CONE object%s\n", (int)numAdded,
          (numAdded != 1 ? "s" : "y"));
 ...
 * @return true if the cache entrie(s) could be added.
 */
bool appendASPOLICYData(char* arg, bool fromFile)
{
  size_t  numBefore, numAdded;
  bool    succ;
  ...
  changeReadToWriteLock(&cache.lock);
  succ = readASPOLICYData(arg, &cache.entries, cache.maxSerial + 1, fromFile);
  changeWriteToReadLock(&cache.lock);
...
/*
 * This method adds the AS-CONE cache entry into the test harness. The format
 * is <provider-AS> <customer-AS> [ <customer-AS>*]
 *
 * The serial notify notification will be send out to all attached clients right
 * away if now==true
 *
 ...
 * @since 0.6.0.0
 */
int _appendASCONE(char* line, bool now)
{
  if (!appendASCONEData(line, false))
  {
    _printAppendError("AS-CONE object", line);
  }
  else if (now)
  {
    sendSerialNotifyToAllClients();
...
/**
 * This method adds the AS-CONE cache entry into the test harness. The format
 * is <provider-AS> <customer-AS>  <verified> [ <customer-AS> <verified> ]*
 *
 * @param line The command line
 *
 * @return CMD_ID_ADD
 */
int appendASCONE(char* line)
{
  return _appendASCONE(line, false);
}
 ...
 * @return CMD_ID_ADDNOW
 */
int appendASCONENow(char* line)
{
  return _appendASCONE(line, true);
}

/*
 * This method adds the AS-POLICY cache entry into the test harness. The format
 * is <provider-AS> <neighbour-AS> [ <neighbour-AS>*]
 *
 * The serial notify notification will be send out to all attached clients right
 * away if now==true
 *
 ...
 *
 */
int _appendASPOLICY(char* line, bool now)
{
  if (!appendASPOLICYData(line, false))
  {
    _printAppendError("AS-POLICY object", line);
  }
  else if (now)
  {
    sendSerialNotifyToAllClients();
 ...
 * @return CMD_ID_ADD
 */
int appendASPOLICY(char* line)
{
  return _appendASPOLICY(line, false);
}
 ...
 * @return CMD_ID_ADDNOW
 */
int appendASPOLICYNow(char* line)
{
  return _appendASPOLICY(line, true);
}
  ...
  else
  {
    FOREACH_SLIST(&cache.entries, lnode)
    {
      ...
      else if (cEntry->isASPA)
      {
        ...
        for (idx = 0; idx < ntohs(cEntry->customerCount); idx++, ptr32++)
        {
      ...
      {
        //@TODO: Output the ASCONE Identifier-String!!
        printf("[ASCONE]: ");
        printf ("PAS=%u", ntohl(cEntry->asNumber));
        
        ptr32 = (uint32_t*)cEntry->providerAS;
        for (idx = 0; idx < ntohs(cEntry->customerCount); idx++, ptr32++)
        {
          printf (", CAS=%u", ntohl(*ptr32));
        }
      }
      else if (cEntry->isASPOLICY)
      {
        ...
        ptr32 = (uint32_t*)cEntry->providerAS;
        for (idx = 0; idx < ntohs(cEntry->neighbourCount); idx++, ptr32++)
        {
          printf (", NAS=%u", ntohl(*ptr32));
        }
      }
      else
      {
...
int handleLine(char* line)
{
  ...
  CMD_CASE("addASCONE", appendASCONE);
  CMD_CASE("addASCONENow", appendASCONENow);
  CMD_CASE("addASPOLICY", appendASPOLICY);
  CMD_CASE("addASPOLICYNow", appendASPOLICYNow);
