                            ...
                            void* userPtr)
{
  ...
  bool printASCONE    = (valType & SRX_PROXY_RESTYPE_ASCONE) != 0;
  char* resultStr;  
  ...
  if (printASCONE)
  {
    switch (asconeResult)
    {
      case ASCONE_RESULT_VALID:
        resultStr = "VALID";
        break;        
      case ASCONE_RESULT_INVALID:
        resultStr = "INVALID";
        break;        
      case ASCONE_RESULT_UNDEFINED:
        resultStr = "UNDEFINED";
        break;        
      case ASCONE_RESULT_UNKNOWN:
        resultStr = "UNKNOWN";
        break;        
      case ASCONE_RESULT_UNVERIFIABLE:
        resultStr = "UNVERIFIABLE";
        break;                
      default:
        resultStr = "???????";
  ...
  if (!(printROA || printBGPSEC || printASPA || printASCONE))
  {
    PRINTF("ERROR - no ROA, BGPSEC, or ASPA, or ASCONE information!!!");
    retVal=false;
  }
...
void doVerify(bool log, char** argPtr)
{
  ...
  verifyUpdate(proxy, localID,
               ...
               (method & SRX_FLAG_BGPSEC) == SRX_FLAG_BGPSEC, false, false,
