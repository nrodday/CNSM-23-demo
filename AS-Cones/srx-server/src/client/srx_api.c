                         ...
                         SRxASPathList asPathList)
{
  ...
  hdr->common.asconeResSrc  = defaultResult->resSourceASCONE;
  ...
  hdr->common.asconeDefRes  = defaultResult->result.asconeResult;
...
void verifyUpdate(SRxProxy* proxy, uint32_t localID,
                  bool usePrefixOriginVal, bool usePathVal, bool useAspaVal, bool useAsconeVal,
                  ...
                  BGPSecData* bgpsec, SRxASPathList asPathList)
{
  ...
  uint8_t method =   (usePrefixOriginVal ? SRX_FLAG_ROA : 0)
                   ...
                   | (useAsconeVal ? SRX_FLAG_ASCONE : 0)
                   | (localID != 0 ? SRX_FLAG_REQUEST_RECEIPT : 0);

  printf("Debug: ASCONE - verifyUpdate\n");
  printf("Method: %d\n", method);
  printf("usePrefixOriginVal: %d\n", usePrefixOriginVal);
  printf("usePathVal: %d\n", usePathVal);
  printf("useAspaVal: %d\n", useAspaVal);
  printf("useAsconeVal: %d\n", useAsconeVal);
  printf("localID: %d\n", localID);
  if (prefix->ip.version == 4)
  {
    printf("Debug: verifyUpdate - prefix->addr: %u.%u.%u.%u\n", prefix->ip.addr.v4.u8[0],prefix->ip.addr.v4.u8[1],prefix->ip.addr.v4.u8[2],prefix->ip.addr.v4.u8[3]);  
  }
  ...
  if (isV4)
  {
    printf("Debug: ASCONE - verifyUpdate - createV4Request - length: %u localID: %08X\n", length, localID);
    createV4Request(pdu, method, requestToken, defaultResult, prefix, as32, bgpsec, asPathList);
  }
  else
  {
    printf("Debug: ASCONE - verifyUpdate - createV6Request\n");
  ...
  do
  {
    ...
    if(sendPacketToServer(connHandler, (SRXPROXY_PDU*)pdu, length))
    {
      ...
      printf("Debug: ASCONE - verifyUpdate- length: %u - sendPakcteToServer OK!!\n", length);
      break;
    }
    else
    {
      ...
      printf("Debug: ASCONE - verifyUpdate - sendPakcteToServer ERROR\n");
    }
...
{
  printf("Debug: processVerifyNotify\n");
  ...
  if (proxy->resCallback != NULL)
  {
    ...
    bool useASCONE    = (hdr->resultType & SRX_FLAG_ASCONE) == SRX_FLAG_ASCONE;
    ...
    printf("resultType: %u\n", hdr->resultType);
    printf("roaResult: %u\n", hdr->roaResult);
    printf("bgpsecResult: %u\n", hdr->bgpsecResult);
    printf("aspaResult: %u\n", hdr->aspaResult);
    printf("asconeResult: %u\n", hdr->asconeResult);
    printf("\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n\n");
    ...
    printf("Debug: hasReceipt: %d, useASPA: %u,  useASCONE: %u, resultType: %u\n", hasReceipt, useASPA, useASCONE, hdr->resultType);
    ...
    uint8_t asconeResult = useASCONE ? hdr->asconeResult : SRx_RESULT_UNDEFINED;
    ...
    printf("type: %u\n", hdr->type);
    printf("resultType: %u\n", hdr->resultType);
    printf("ntohs_resultType: %u\n", ntohs(hdr->resultType));
    printf("roaResult: %u\n", hdr->roaResult);
    printf("bgpsecResult: %u\n", hdr->bgpsecResult);
    printf("aspaResult: %u\n", hdr->aspaResult);
    printf("ntohs_aspaResult: %u\n", ntohs(hdr->aspaResult));
    printf("asconeResult: %u\n", hdr->asconeResult);
    printf("\n\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n\n");
    ...
    if (useASCONE)
    {
      printf("Debug: ASCONE SRX asconeResult: %u, useASCONE: %u, hdr->asconeResult: %u, localID: %u, updateID: %08X\n", asconeResult, useASCONE, hdr->asconeResult, localID, updateID);
    }

    if (useASPA)
    {
      printf("Debug: ASPA SRX aspaResult: %u, useAPA: %u, hdr->asPAResult: %u, localID: %u, updateID: %08X\n", aspaResult, useASPA, hdr->aspaResult, localID, updateID);
    }

    ValidationResultType valType = hdr->resultType & SRX_FLAG_ROA_BGPSEC_ASPA_ASCONE;
    printf("Debug: valType: %u hdr->resultType: %u\n", valType, hdr->resultType);


    // hasReceipt ? localID : 0 is result of BZ263
    proxy->resCallback(updateID, localID, valType, roaResult, bgpsecResult,
                       asconeResult, aspaResult, proxy->userPtr); // call handleSRxValidationResult
  }
  else
  {
    printf("Debug: processVerifyNotify: NO IMPLEMENTATION PROVIDED FOR\n");
